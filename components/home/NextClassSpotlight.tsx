...
import { ArrowRight, User } from "@/components/ui/SiteIcons";
 ...
 *
 * Real, confirmed gap this fixes: getDisplayedPreDraftScore was
 * called with no format, always defaulting to the wrong format before the global preference loaded —
 * meaning not just the displayed number but the *ranking order
 * itself* (who's #1, #2, #3) never actually reflected format, even
 * though a QB-heavy class could genuinely reorder in Superflex. No
 * format toggle lives on this specific section, so — same pattern as
 * ModelTrackRecord — this now reads the site's cross-page sticky
 * format preference instead of a fixed baseline.
 */
export function NextClassSpotlight({ prospects, classYear }: { prospects: Prospect[]; classYear?: string }) {
  ...
  const top3 = prospects
    ...
    .map((p) => ({ prospect: p, score: getDisplayedPreDraftScore(p, format) }))
  ...
  return (
    <section className="border-b border-border bg-surface py-12 sm:py-24">
      <Container>
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-base leading-relaxed text-ink-secondary">
              See the prospects currently leading the {classYear} class, with live scores that update as new information enters the model.
            </p>
          </div>
          <Link
            href={`/classes/${classYear}`}
            prefetch={false}
            className="group flex shrink-0 items-center gap-1.5 font-headline text-sm uppercase text-accent hover:underline"
          >
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-5">
          ...
          <Link
            href={playerHref(featured.prospect.id, format)}
            prefetch={false}
