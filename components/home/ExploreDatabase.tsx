...
import { ArrowUpRight } from "@/components/ui/SiteIcons";
...
export function ExploreDatabase({ prospects }: { prospects: Prospect[] }) {
  ...
  return (
    <section id="explore" className="theme-dark border-b border-border bg-void py-12 sm:py-24">
      <Container>
        <div className="max-w-2xl">
          <span className="font-headline text-sm uppercase text-accent">Explore the Database</span>
        ...
        <div className="mt-12">
          <p className="font-headline text-sm uppercase text-ink-tertiary">By Position</p>
        ...
        {years.length > 0 && (
          <div className="mt-10">
            <p className="font-headline text-sm uppercase text-ink-tertiary">By Class Year</p>
            <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
              {years.map((year) => {
                const count = counts.get(year) ?? 0;
                return (
                  <Link key={year} href={`/classes/${year}`} prefetch={false} className="block">
                    <CornerFrame className="flex h-full flex-col justify-between transition-transform duration-200 hover:-translate-y-1">
