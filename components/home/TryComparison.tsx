...
export function TryComparison({ defaultA, defaultB }: { defaultA?: Prospect; defaultB?: Prospect }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => { if (entries.some(e => e.isIntersecting)) { setNearby(true); observer.disconnect(); } }, { rootMargin: "300px" });
  ...
  useEffect(() => {
    const sync = (event?: Event) => setFormat((event as CustomEvent<LeagueFormat> | undefined)?.detail ?? getGlobalFormat());
    sync();
  ...
  return (
    <section ref={sectionRef} className="theme-dark border-b border-border bg-void py-12 sm:py-24">
      {loadError && <p role="alert" className="p-4 text-sm">Comparison could not load. <Link href="/compare" className="underline">Open comparison tool</Link></p>}
      <Container>
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="max-w-2xl">
            <span className="font-headline text-sm uppercase text-accent">Try It</span>
        ...
        <div className="mt-10 border border-border-strong bg-surface p-5 sm:p-8">
          ...
          {prospects.length >= 2 && (
            <button
              onClick={randomize}
              className="mt-6 flex items-center gap-1.5 text-[10px] text-ink-tertiary transition-colors duration-150 hover:text-accent"
            >
