...
export function Trending({ risers, fallers }: { risers: ScoreMover[]; fallers: ScoreMover[] }) {
  ...
  return (
    <section className={`border-b border-border bg-surface ${hasMovers ? "py-12 sm:py-24" : "py-14"}`}>
      <Container>
        <div className="max-w-2xl">
          <span className="font-headline text-sm uppercase text-accent">Pre-Draft Movers</span>
        ...
        {!hasMovers ? (
          ...
          <div className="mt-8 flex flex-wrap items-center gap-x-4 gap-y-2">
            ...
            <Link href="/players" className="shrink-0 font-headline text-sm uppercase text-accent hover:underline">
            ...
            {risers.length > 0 && (
              <div className="border border-border bg-void">
                <div className="flex items-center gap-2 border-b border-border px-4 py-3">
                  ...
                  <span className="font-headline text-sm uppercase text-ink-tertiary">Risers</span>
                </div>
                {risers.map((m) => (
            ...
            {fallers.length > 0 && (
              <div className="border border-border bg-void">
                <div className="flex items-center gap-2 border-b border-border px-4 py-3">
                  ...
                  <span className="font-headline text-sm uppercase text-ink-tertiary">Fallers</span>
                </div>
                {fallers.map((m) => (
