...
import { Target, Database, Layers, TrendingUp } from "@/components/ui/SiteIcons";
import { Container } from "@/components/layout/Container";
import { computeCombinedHitRate, overallStats, selectionFromLeagueFormat } from "@/lib/analytics";
 ...
 * a copy bug.
 *
 * Real, confirmed gap this fixes: computeCombinedHitRate accepts a
 * format parameter (Elite+ hit rate genuinely differs by league
 * format, the same way it does on every Position page), but this
 * call site never passed one, briefly rendering the wrong default before the global preference loaded
 * of the site's actual sticky format preference. This page has no
 * format toggle of its own, so — same pattern used elsewhere on the
 * homepage — it now reads the same cross-page sticky preference
 * every other listing page respects, rather than a fixed baseline.
 */
export function ModelTrackRecord({ prospects }: { prospects: Prospect[] }) {
  ...
  const topTier = computeCombinedHitRate(prospects, ["Generational", "Elite"], selectionFromLeagueFormat(format));

  const modelPositions = new Set(prospects.filter((p) => p.position === "QB" || p.position === "RB" || p.position === "WR" || p.position === "TE").map((p) => p.position)).size;
  ...
  return (
    <section className="border-b border-border bg-void py-12 sm:py-24">
      <Container>
        <div className="max-w-2xl">
          <span className="font-headline text-sm uppercase text-accent">Track Record</span>
          ...
          <p className="mt-4 text-base leading-relaxed text-ink-secondary">
            Every outcome of every player is tracked to keep the model accountable. This is where it currently stands.
          </p>
        </div>
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-5">
          ...
          {topTier && (
            <div className="relative flex flex-col justify-center overflow-hidden border border-border-strong bg-surface p-8 lg:col-span-2">
              ...
              <p className="mt-2 font-headline text-sm uppercase text-ink-tertiary">
                Elite+ Tier Hit Rate
              </p>
          ...
          <div className={`grid grid-cols-1 gap-px overflow-hidden border border-border bg-border sm:grid-cols-3 ${topTier ? "lg:col-span-3" : "lg:col-span-5"}`}>
            {secondaryItems.map((item) => (
              <div key={item.label} className="flex flex-col bg-surface p-6">
                ...
                <p className="mt-1 font-headline text-sm uppercase text-ink-tertiary">{item.label}</p>
