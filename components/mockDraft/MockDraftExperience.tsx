...
// Everything below is content that used to be defined inline in this
// same file, before it was split up (this file was 2,200+ lines, by
// far the largest component on the site) — see each import's own
// file for why it was a safe, self-contained piece to move.
import { AvailablePlayerRow } from "@/components/mockDraft/AvailablePlayerRow";
import { Segment, EngineCard, DraftTypeCard, Stat } from "@/components/mockDraft/MockDraftSetupControls";
...
import {
  ...
  engineLabelShort,
  parsePickTimer,
  formatPickTimerLabel,
  formatCountdown,
  playerRankLabel,
  POSITION_CLASS,
  POSITION_DOT,
  POSITION_ORDER,
} from "@/lib/mockDraftFormat";
...
export function MockDraftExperience({
  ...
  const [sessionProspects, setSessionProspects] = useState<Prospect[] | null>(null);
  ...
  const initialProspects = useMemo(
    () => sessionProspects ?? classProspectsByYear[classYear] ?? [],
    [classProspectsByYear, classYear, sessionProspects]
  );
    ...
    fetch("/api/sleeper?action=saved")
      ...
      .then((data) => {
        ...
        return fetch(`/api/sleeper?action=needs&leagueId=${mostRecent.leagueId}&rosterId=${mostRecent.rosterId}`)
          .then((res) => res.json())
          .then((needsData) => {
            if (!needsData.needs || stepRef.current !== "setup") return;
  ...
  useEffect(() => {
    if (syncedNeeds || stepRef.current !== "setup") return;
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("dd-mock-checkpoint-v1");
      if (raw) {
        const saved = JSON.parse(raw);
        if (!validDraftCheckpoint(saved) || saved.version !== 1 || !classProspectsByYear[saved.classYear] || !Array.isArray(saved.prospects) || !Array.isArray(saved.picks) || !saved.settings || ![8,10,12,14,16].includes(saved.settings.teams) || saved.picks.length > saved.settings.teams * 4) throw new Error("Invalid checkpoint");
        const ids = new Set(saved.prospects.map((p: Prospect) => p.id));
        if (new Set(saved.picks.map((p: MockPick) => p.playerId)).size !== saved.picks.length || saved.picks.some((p: MockPick, i: number) => p.overall !== i + 1 || !ids.has(p.playerId))) throw new Error("Invalid picks");
        setClassYear(saved.classYear); if (saved.step !== "setup" || saved.picks.length) setSessionProspects(saved.prospects);
        setTeams(saved.settings.teams); setQbFormat(saved.settings.qbFormat); setTeFormat(saved.settings.teFormat);
        setSlot(saved.settings.slot); setEngine(saved.settings.engine); setPickTimer(saved.settings.pickTimer);
        setPicks(saved.picks); picksRef.current = saved.picks;
        setQueue(Array.isArray(saved.queue) ? saved.queue.filter((id: string) => ids.has(id)) : []);
        setMode(saved.mode === "existing" ? "existing" : "new"); setExistingPickIndex(saved.existingPickIndex ?? 0);
        setRealOrderPicks(saved.realOrderPicks ?? null); setSyncedNeeds(saved.syncedNeeds ?? null); setUseSyncedNeedsForDraft(!!saved.useSyncedNeedsForDraft);
        const restoredStep = saved.step === "results" ? "results" : saved.step === "setup" ? "setup" : "draft";
        stepRef.current = restoredStep; setStep(restoredStep); setPaused(restoredStep === "draft");
        setDraftId(typeof saved.id === "string" ? saved.id : crypto.randomUUID());
        setRecoveryNotice(restoredStep === "draft" ? "Draft restored and paused. Resume when ready." : "Your previous draft has been restored.");
      } else setDraftId(crypto.randomUUID());
    } catch { setRecoveryNotice("The previous draft could not be restored. Start a new draft below."); setDraftId(crypto.randomUUID()); }
    setCheckpointReady(true);
    // Restore only on entry; sheet refreshes must not replace a running session.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (!checkpointReady || mode === null) return;
    if (!sessionProspects && (step !== "setup" || picks.length > 0)) setSessionProspects(initialProspects);
    try {
      localStorage.setItem("dd-mock-checkpoint-v1", JSON.stringify({ version: 1, id: draftId, classYear, settings, picks, queue, mode, step, existingPickIndex, realOrderPicks, syncedNeeds, useSyncedNeedsForDraft, prospects: sessionProspects ?? initialProspects }));
    } catch { setRecoveryNotice("Browser storage is unavailable. Keep this tab open until your draft is saved to your account."); }
  }, [checkpointReady, mode, draftId, classYear, settings, picks, queue, step, existingPickIndex, realOrderPicks, syncedNeeds, useSyncedNeedsForDraft, initialProspects, sessionProspects]);
  ...
  // Historical tier rates keyed by score stage, position and tier,
  // matching the league format selected for this mock draft.
  ...
  const resetDraft = useCallback(() => {
    if (picksRef.current.length && !window.confirm("Start a new draft? This clears the draft stored in this browser. Account-saved results remain available.")) return;
    try { localStorage.removeItem("dd-mock-checkpoint-v1"); } catch {}
    setSessionProspects(null); setDraftId(crypto.randomUUID()); setRecoveryNotice("");
  ...
  const ddValueCaptured = useMemo(() => computeDDValueCaptured(
  ...
  if (!initialProspects.length) {
    return (
      <div className="border border-border bg-surface p-8 text-center">
        <p className="font-headline text-sm uppercase text-ink-tertiary">No class data</p>
  ...
  if (!checkpointReady) return <p className="p-6 text-sm text-ink-tertiary">Loading draft…</p>;
      ...
      <section className="mock-draft-setup mx-auto max-w-6xl px-0 pb-10 lg:pb-10">
        ...
        <div className="relative mb-5 overflow-hidden border-b border-border px-4 pb-5 sm:mb-7 sm:px-0 sm:pb-6">
          <div className="relative flex items-end justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-[10px] font-semibold text-accent"><span className="h-px w-7 bg-accent" /> Mock Draft Engine</div>
        ...
        <div className="grid gap-5 lg:gap-6 lg:grid-cols-[1.05fr_.95fr]">
          <div className="border border-border bg-surface p-5 sm:p-6">
            <div className="flex items-center gap-2 text-[10px] font-semibold text-ink-tertiary">
            ...
            <div className="mt-4 space-y-4 sm:mt-5 sm:space-y-5">
              ...
              {useRealPickOrder && realOrderPicks ? (
                <div className="border border-accent/30 bg-accent/5 p-3">
                  <p className="text-[10px] font-semibold text-ink-tertiary">Teams</p>
              ...
              {useRealPickOrder && realOrderPicks ? (
                <div className="border border-accent/30 bg-accent/5 p-3">
                  <p className="text-[10px] font-semibold text-ink-tertiary">QB format</p>
              ...
              {useRealPickOrder && realOrderPicks ? (
                <div className="border border-accent/30 bg-accent/5 p-3">
                  <p className="text-[10px] font-semibold text-ink-tertiary">TE premium</p>
              ...
              {useRealPickOrder && realOrderPicks ? (
                <div className="border border-accent/30 bg-accent/5 p-3">
                  <p className="text-[10px] font-semibold text-ink-tertiary">Your picks</p>
                ...
                <div>
                  <label className="mb-2 block text-[10px] font-semibold text-ink-tertiary">Your draft slot</label>
              ...
              <div>
                <label className="mb-2 block text-[10px] font-semibold text-ink-tertiary">Pick timer</label>
                <select
          ...
          <div className="border border-border bg-surface p-5 sm:p-6">
            ...
            <div className="mt-4 border-t border-border pt-4 sm:mt-5 sm:pt-5">
              <div className="flex items-center gap-2 text-[10px] font-semibold text-ink-tertiary"><span className="h-5 w-5 border border-border-strong bg-surface-raised text-center leading-5 text-accent">3</span> Draft type</div>
  ...
  return (
    <section className="mock-draft-shell fixed inset-x-0 bottom-0 top-[var(--nav-offset)] z-40 flex min-h-0 flex-col overflow-hidden bg-void sm:static sm:z-auto sm:block sm:min-h-0 sm:overflow-visible">
      {recoveryNotice && <p role="status" className="shrink-0 border-b border-border px-4 py-2 text-xs text-ink-secondary">{recoveryNotice} <button onClick={() => setRecoveryNotice("")} className="underline">Dismiss</button></p>}
      {mode === "existing" && existingPickIndex < maxPicks && existingPickIndex < manualEntryLimit && (
        <div className="mb-4 border border-accent/30 bg-accent/5 p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-semibold text-accent">Continue Existing Draft</p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_280px]">
            ...
            <div className="border border-border bg-surface p-3">
              <p className="font-headline text-sm uppercase text-ink-tertiary">Already entered</p>
      ...
      <div className="mock-draft-toolbar shrink-0 flex flex-col gap-1.5 border-b border-border bg-surface px-3 py-2 sm:mb-4 sm:flex-row sm:items-center sm:justify-between sm:border sm:border-border-strong sm:px-5 sm:py-4">
        <div className="flex items-center gap-3">
          <button onClick={resetDraft} title={recoveryNotice || "Back to draft setup"} aria-label="Back to draft setup" className="flex h-9 w-9 items-center justify-center border border-border-strong text-ink-secondary hover:border-accent hover:text-accent"><ArrowLeft className="h-4 w-4" /></button>
        ...
        >
          ...
          {pickTimer !== "UNTIMED" && isUserTurn && remainingSeconds !== null ? (
            <>
              ...
              <span className="font-headline text-sm uppercase text-ink-tertiary">per pick</span>
            </>
          ) : (
      ...
      <div className="mock-draft-layout flex min-h-0 min-w-0 flex-1 flex-col gap-0 lg:grid lg:grid-cols-[minmax(0,1.45fr)_minmax(380px,.9fr)] lg:gap-4">
        <div className="mock-draft-board-panel relative flex h-[48%] min-h-0 min-w-0 shrink-0 flex-col overflow-hidden border-b border-border bg-surface-raised lg:h-auto lg:min-h-0 lg:border lg:border-border-strong">
          ...
          <div className="flex shrink-0 items-center justify-between border-b border-border bg-surface px-3 py-2 sm:px-4 sm:py-2.5">
            ...
            <div className="flex items-center gap-1 font-headline text-sm uppercase text-ink-tertiary">
                  ...
                  <div key={team} className={cn("snap-start border-b border-r border-border bg-surface-raised px-1.5 py-1.5", team === slot && "mock-user-draft-column bg-accent/5")}>
                    <div className="flex items-center justify-between gap-2">
                      <span className="flex min-w-0 items-center gap-2">
                        ...
                        <span className="truncate text-[10px] font-semibold text-ink-secondary">Team {team}</span>
                      </span>
                      {team === slot && <span className="text-[10px] text-accent">You</span>}
                    </div>
                  </div>
                ))}
                        ...
                        {player ? (
                          ...
                          <div className="relative flex animate-fade-in items-start gap-1 pt-7 lg:block lg:pt-8">
                            <div className="min-w-0 flex-1 lg:pr-16">
                              ...
                              <p className="mt-1 font-data text-[10px] font-semibold" style={{ color: pickTier ? getTierColor(pickTier) : undefined }}>{tierHistoryLabel(player, "mock")} {getScoreForFormat(player, qbFormat, teFormat)?.toFixed(1) ?? "TBD"}</p>
                            </div>
                            {player.photoUrl && (
                              <Image
                                src={player.photoUrl}
                          ...
                          <div className="flex h-full min-h-[40px] items-center justify-center">
                            <div className="text-center">
                              ...
                              <p className="mt-1 text-[10px] font-semibold text-accent">{isUserTurn ? "Your pick" : "On the clock"}</p>
                            </div>
                          </div>
                        ) : (
          ...
          <div className="hidden border-t border-border bg-surface-raised px-4 py-3 lg:block">
            {paused ? (
              ...
              <div className="flex items-center justify-between gap-3">
                <div><p className="text-[10px] text-accent">On the clock</p><p className="mt-1 text-sm font-semibold text-ink">Your pick: {formatPick(currentOverall, teams)}</p></div>
                {pickTimer === "UNTIMED" || remainingSeconds === null ? (
                  <span className="text-[10px] text-ink-tertiary">No timer</span>
                ) : (
                  <span className={cn("font-data text-2xl font-bold", remainingSeconds <= 10 ? "text-faller" : "text-ink")}>
                    {formatCountdown(remainingSeconds)}
                  </span>
        ...
        <div className={cn("mock-player-panel relative z-20 flex min-h-0 min-w-0 flex-col border-t border-border bg-surface lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-hidden lg:border", mobilePlayersExpanded ? "mobile-expanded" : "h-[52%]", "lg:static lg:h-auto lg:shadow-none")}>
          ...
          {/* pb-6 alone (24px, no safe-area awareness) wasn't enough:
              this whole shell is `fixed ... bottom-0`, flush to the
              actual viewport edge rather than routed through the
              site's global pb-24 wrapper (which every other page
              relies on to clear the safe area) — that wrapper can't
              reach in here regardless, since fixed positioning
              ignores ancestor padding. On a notched/home-indicator
              phone the last visible pick in this list — Players,
              Queue, or My Team, all three share this one scroll
              container — could end up sitting right under or partly
              behind the home-indicator gesture area. */}
          <div className="mock-player-list min-h-0 flex-1 overflow-y-auto overscroll-y-contain pb-[calc(1.5rem+env(safe-area-inset-bottom))] [scrollbar-gutter:stable] [-webkit-overflow-scrolling:touch] lg:h-[calc(100vh-15rem)] lg:max-h-[calc(100vh-15rem)] lg:pb-6">
                ...
                {activeSyncedNeeds && !activeSyncedNeeds.bpaMode && (() => {
                  ...
                  return (
                    <div className="border-b border-accent/30 bg-accent/5 px-3 py-2 sm:px-4">
                      <span className="text-[10px] font-semibold text-accent">Still needed: </span>
                  ...
                  return (
                    <div key={pos}>
                      <div className="flex items-center justify-between border-b border-border bg-surface-raised px-3 py-1.5 sm:px-4">
                        <span className="text-[10px] font-semibold text-ink-tertiary">{pos}</span>
                      ...
                      {group.map((p) => (
                        <div key={p.id} className="flex items-center gap-3 border-b border-border px-3 py-2 sm:px-4">
                          ...
                          <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-ink">{p.name}</p><p className="mt-1 text-[10px] text-ink-tertiary">{p.school ?? ""} · {tierHistoryLabel(p, "mock")} {getScoreForFormat(p, qbFormat, teFormat)?.toFixed(1) ?? "TBD"}</p></div>
                        </div>
                      ))}
                    </div>
                  );
                  ...
                  return (
                    <div key={id} className="flex items-center gap-3 border-b border-border px-3 py-2 sm:px-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[10px] text-ink-tertiary">{qp.position} · {qTier ?? "TBD"} · {tierHistoryLabel(qp, "mock")} {qScore?.toFixed(1) ?? "TBD"}</p>
                      </div>
              ...
              return (
                <Fragment key={p.id}>
                  {showTierBreak && (
                    ...
                    >
                      <span className="text-[10px] font-semibold" style={{ color: tier ? getTierColor(tier) : undefined }}>
                        {tier}
                      </span>
                    </div>
                  )}
          ...
          <div className="hidden border-t border-border bg-surface-raised px-4 py-3 lg:block">
            {engine === "COMMUNITY" && (communitySource === "loading" || communitySource === "unavailable") ? (
              <div className="mb-2 text-[10px] text-ink-tertiary">Community board: {getCommunityFormatLabel(qbFormat, teFormat)} · Source: FantasyCalc</div>
            ) : null}
      ...
      {(expandedPick || expandedPlayerId) && (
        <DialogSurface onClose={() => { setExpandedPick(null); setExpandedPlayerId(null); }} aria-label="Player details"  className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto px-4 pb-8 pt-20 sm:items-center sm:py-12">
          ...
          <div className="relative z-10 max-h-[85dvh] overflow-y-auto w-full max-w-2xl shrink-0 self-start animate-fade-in-up border border-border-strong bg-surface p-5 shadow-2xl [animation-duration:200ms] sm:self-center sm:p-7">
                ...
                <div>
                  ...
                  <div className={cn(pick && "mt-3")}>
                    <MockDraftPlayerCard
                      ...
                      tier={pickTier}
                      tierSampleSize={pickTier ? positionTierHitRates.get(`sample:${playerScoreStage(prospect, "mock")}:${prospect.position}:${pickTier}`) : undefined}
                      tierHitRate={pickTier ? positionTierHitRates.get(`${playerScoreStage(prospect, "mock")}:${prospect.position}:${pickTier}`) : undefined}
