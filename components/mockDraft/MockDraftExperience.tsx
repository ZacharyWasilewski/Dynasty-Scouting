...
function AvailablePlayerRow({
  ...
  return (
    ...
    >
      ...
      <div className="mock-player-stat order-4 w-14 shrink-0 max-md:w-20 max-md:min-w-[80px] md:order-none border-l border-border px-3"><p className="whitespace-normal break-words text-[10px] text-ink-tertiary max-md:whitespace-nowrap max-md:break-normal">DD Score</p><p className="mt-0.5 whitespace-normal break-words text-xs font-semibold text-ink max-md:whitespace-nowrap max-md:break-normal">{score?.toFixed(1) ?? "TBD"}</p></div>
      <div className="mock-player-stat order-5 w-14 shrink-0 max-md:w-20 max-md:min-w-[80px] md:order-none border-l border-border px-3"><p className="whitespace-normal break-words text-[10px] text-ink-tertiary max-md:whitespace-nowrap max-md:break-normal">Hit Rate</p><p className="mt-0.5 whitespace-normal break-words text-xs font-semibold text-ink max-md:whitespace-nowrap max-md:break-normal">{tier && positionTierHitRates.get(`${p.position}:${tier}`) !== undefined ? `${positionTierHitRates.get(`${p.position}:${tier}`)!.toFixed(0)}%` : "—"}</p></div>
      <div className="mock-player-stat order-6 w-14 shrink-0 max-md:w-20 max-md:min-w-[80px] md:order-none border-l border-border px-3"><p className="whitespace-normal break-words text-[10px] text-ink-tertiary max-md:whitespace-nowrap max-md:break-normal">COMM</p><p className={cn("mt-0.5 whitespace-normal break-words text-xs font-semibold max-md:whitespace-nowrap max-md:break-normal", diff === null ? "text-ink-tertiary" : diff > 0 ? "text-riser" : diff < 0 ? "text-faller" : "text-ink-secondary")}>{cr ?? "—"}</p></div>
      {/* Draft Proj and School removed — reported directly as
          ...
          {p.subScores?.filter((s): s is typeof s & { value: number } => s.value !== undefined).map((s) => (
            <div key={s.label} className="mock-player-stat order-7 w-14 shrink-0 max-md:w-20 max-md:min-w-[80px] md:order-none border-l border-border px-3">
              <p className="whitespace-normal break-words text-[10px] text-ink-tertiary max-md:whitespace-nowrap max-md:break-normal">{s.label}</p>
...
function ResultsScreen({ classYear, settings, picks, prospectById, gradeRows, overallGrade, ddValueCaptured, onReset, onViewBoard }: { classYear: string; settings: MockSettings; picks: MockPick[]; prospectById: Map<string, Prospect>; gradeRows: Array<{ pick: MockPick; player: Prospect; grade: string; valueGain: number; scoreGap: number; tierGap: number }>; overallGrade: string; ddValueCaptured: number; onReset: () => void; onViewBoard: () => void }) {
  ...
  return <section className="mx-auto max-w-4xl"><div className="mb-5 flex items-center justify-between gap-4 px-4 sm:px-0"><div><p className="font-headline text-sm uppercase text-accent">Draft complete</p><h2 className="mt-1 font-headline text-3xl uppercase leading-none text-ink">Your {classYear} Draft</h2>{user ? (
