      ...
      {selected ? (
        <div className="mt-2 flex items-center justify-between gap-3 border border-border-strong bg-surface p-4">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-ink-tertiary">
              {selected.position} · {selected.school ?? "—"} · {selected.draftClass ?? "—"}
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="relative mt-2">
          ...
          <input
            ...
            className="w-full border border-border-strong bg-surface py-3 pl-10 pr-4 text-sm text-ink placeholder:text-ink-tertiary focus:border-accent/50 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
          />

          {showDropdown && (
            <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-72 overflow-y-auto border border-border-strong bg-surface shadow-xl">
              {results.length === 0 ? (
