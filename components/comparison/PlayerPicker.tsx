...
export function PlayerPicker({
  ...
  return (
    <div className="relative">
      <span className="font-headline text-sm uppercase text-ink-tertiary">{label}</span>
      ...
      {selected ? (
        ...
        <div className="relative mt-2">
          ...
          <input
            ...
            onKeyDown={e => { if (e.key === "Escape") { setFocused(false); return; } if (e.key === "ArrowDown") { e.preventDefault(); inputRef.current?.parentElement?.querySelector<HTMLButtonElement>("button")?.focus(); } }}
            onFocus={() => setFocused(true)}
            onBlur={(e) => { if (!e.currentTarget.parentElement?.contains(e.relatedTarget)) setFocused(false); }}
          ...
          {showDropdown && (
            <div id={resultsId} role="listbox" aria-label={`${label} results`} className="absolute left-0 right-0 top-full z-50 mt-1 max-h-72 overflow-y-auto border border-border-strong bg-surface shadow-xl">
              {results.length === 0 ? (
                ...
                results.map((p) => (
                  <button
                    ...
                    onKeyDown={e => { if (e.key === "Escape") { inputRef.current?.focus(); setFocused(false); } if (e.key === "ArrowDown" || e.key === "ArrowUp") { e.preventDefault(); const buttons = Array.from(e.currentTarget.parentElement?.querySelectorAll<HTMLButtonElement>("button") ?? []); const index = buttons.indexOf(e.currentTarget); buttons[(index + (e.key === "ArrowDown" ? 1 : buttons.length - 1)) % buttons.length]?.focus(); } }}
                    onClick={() => {
                      onSelect(p);
                      setQuery("");
                    }}
