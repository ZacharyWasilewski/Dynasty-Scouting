...
export function AdminRefreshControl({ initialVersion, initialProspectCount }: Props) {
  ...
  useEffect(() => { setVersion(initialVersion); setProspectCount(initialProspectCount); }, [initialVersion, initialProspectCount]);
  ...
  return (
    <div className="mt-8 border border-border bg-surface p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-headline text-sm uppercase text-ink-tertiary">Data cache</p>
          <p className="mt-2 text-sm text-ink-secondary">
            Snapshot v{version} · {prospectCount} prospects loaded
            {refreshedAt && (
              <>
                {" · "}
                <span className="text-riser">last live refresh: {new Date(refreshedAt).toLocaleString("en-US", { timeZone: "America/Detroit", timeZoneName: "short" })}</span>
              </>
            )}
          </p>
      ...
      {error && (
        <p role="alert" className="mt-3 border border-faller/30 bg-faller/10 px-3 py-2 text-xs text-faller">{error}</p>
      )}
    </div>
  );
}
