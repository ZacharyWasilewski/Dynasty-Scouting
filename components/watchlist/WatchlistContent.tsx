...
export function WatchlistContent({ prospects }: { prospects: Prospect[] }) {
  ...
  const { ids, hydrated, loadError } = useWatchlist();
      ...
      <section className="py-10">
        <Container>
          {authLoading ? (
            ...
            <div className="h-24" />
          ) : loadError ? (<p role="alert" className="text-sm">Could not load your watchlist. <button onClick={() => window.location.reload()} className="underline">Retry</button></p>) : saved.length === 0 ? (
            <EmptyState
              icon={Bookmark}
