...
// AnalyticsPosition, not the wider Position type — matches what
// setPosition (from the shared context) actually expects, since the
// values here only ever are these 4 anyway.
                  ...
                  <div key={s.index} className="flex shrink-0 flex-col items-center gap-2" style={{ width: GROUP_WIDTH }}>
                    <div className="flex items-end gap-2" style={{ height: BAR_AREA_HEIGHT }}>
                      <div className="relative flex flex-col items-center justify-end" style={{ width: BAR_WIDTH, height: BAR_AREA_HEIGHT }}>
                        <span
                          className="absolute whitespace-nowrap font-data text-[10px] text-ink-secondary"
                      ...
                      <div className="relative flex flex-col items-center justify-end" style={{ width: BAR_WIDTH, height: BAR_AREA_HEIGHT }}>
                        <span
                          className="absolute whitespace-nowrap font-data text-[10px] font-semibold text-ink"
                    ...
                    <span
                      className="whitespace-nowrap font-data text-[10px] font-semibold"
                      style={{ color: s.lift! >= 0 ? "var(--color-riser)" : "var(--color-faller)" }}
                    >
                      {s.lift! >= 0 ? "+" : ""}{s.lift!.toFixed(0)} pts
                    </span>
                    <span className="whitespace-nowrap text-center font-tech text-[9px] uppercase tracking-wide text-ink-tertiary">
                      {s.label}
                    </span>
                  </div>
                );
