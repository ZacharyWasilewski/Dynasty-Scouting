...
function ScoringModeInfo() {
  ...
  return (
    <>
      ...
      {open && (
        <DialogSurface onClose={() => setOpen(false)} aria-label="Scoring modes"  className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-void/80 backdrop-blur-sm"
            onClick={() => setOpen(false)}
          />
          <div className="relative z-10 max-h-[85dvh] overflow-y-auto w-full max-w-md border border-border-strong bg-surface p-6 shadow-xl">
            <button
              type="button"
              onClick={() => setOpen(false)}
            ...
            <div className="pr-8">
              ...
              <p className="mt-3 text-sm leading-relaxed text-ink-secondary">
                Standard hit rate is HIT ÷ (HIT + MISS); RP pushes and unresolved outcomes are excluded. Weighted mode averages fantasy-finish values, capped at 100. It is a quality score, not the percentage of players who hit.
              </p>
            </div>
