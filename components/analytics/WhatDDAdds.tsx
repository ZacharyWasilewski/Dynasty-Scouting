...
import { CAPITAL_TIERS, ddScoreForFormat, tierForFormat } from "@/lib/analytics";
...
export function WhatDDAdds({ prospects }: { prospects: Prospect[] }) {
  ...
  const rounds = CAPITAL_TIERS.map(band => historical.filter(p => (p.adp as number) >= band.min && (p.adp as number) <= band.max));
  const roundRows = rounds.map((rows, i) => {
    ...
    return { round: CAPITAL_TIERS[i]!.label, n: rows.length, top: hitRate(top), bottom: hitRate(bottom) };
  }).filter(r => r.n >= 4);

  const lateHits = historical.filter(p => (p.adp as number) > 180 && p.hitMiss === "HIT").sort((a,b) => (ddScoreForFormat(b, selection) ?? 0) - (ddScoreForFormat(a, selection) ?? 0)).slice(0,3);
  const earlyMisses = historical.filter(p => (p.adp as number) <= 32 && p.hitMiss === "MISS").sort((a,b) => (ddScoreForFormat(a, selection) ?? 0) - (ddScoreForFormat(b, selection) ?? 0)).slice(0,3);
  ...
  return (
    <section className="border border-border-strong bg-surface p-6 sm:p-8">
      <span className="font-headline text-sm uppercase text-accent">What DD Adds Beyond Draft Capital</span>
      ...
      <div className="mt-6 grid gap-px border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={title} className="bg-surface p-4">
            <p className="font-headline text-sm uppercase text-accent">{stage}</p>
      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-2">
        <div className="border border-border bg-void/20 p-5">
          <p className="font-headline text-sm uppercase text-ink-tertiary">Does DD separate players within a pick band?</p>
          <p className="mt-2 text-sm leading-relaxed text-ink-secondary">Within each NFL overall-pick band, players are split into the higher and lower halves by DD Score. A gap here means the model is adding ordering information beyond simply knowing the pick band.</p>
          <div className="mt-4 space-y-2">
            {roundRows.map(r => (
              <div key={r.round} className="grid grid-cols-[80px_1fr_auto] items-center gap-3 border-t border-border pt-2 text-xs">
                <span className="font-mono text-ink-tertiary">{r.round}</span>
        ...
        <div className="border border-border bg-void/20 p-5">
          <p className="font-headline text-sm uppercase text-ink-tertiary">Where the model can add context</p>
          ...
          <div className="mt-4 space-y-4">
            <div>
              <p className="text-[10px] text-riser">Hits after pick 180</p>
              <p className="mt-1 text-xs text-ink-tertiary">{lateHits.length ? lateHits.map(p => `${p.name} (${ddScoreForFormat(p, selection)?.toFixed(0)})`).join(" · ") : "No resolved examples in picks 181+ with a DD Score."}</p>
            </div>
            <div>
              <p className="text-[10px] text-faller">Top-32-pick misses</p>
              <p className="mt-1 text-xs text-ink-tertiary">{earlyMisses.length ? earlyMisses.map(p => `${p.name} (${ddScoreForFormat(p, selection)?.toFixed(0)})`).join(" · ") : "No resolved top-32-pick misses in the current sample."}</p>
            </div>
            <div>
              <p className="text-[10px] text-ink-tertiary">Pre-draft sample</p>
