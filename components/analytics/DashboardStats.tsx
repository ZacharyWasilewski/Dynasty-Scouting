...
export function DashboardStats({ prospects, lastUpdated }: { prospects: Prospect[]; lastUpdated?: Date | null }) {
    ...
    <div>
      ...
      {items.map((item) => (
        <div key={item.label} className="flex min-h-[142px] flex-col items-center justify-center bg-surface p-4 text-center sm:min-h-[164px] sm:p-6">
          <span className="max-w-full font-headline text-sm uppercase leading-relaxed text-ink-tertiary">
            {item.label}
          </span>
      ...
      {lastUpdated && <DataFreshness lastUpdated={lastUpdated} className="mt-2 block" />}
    </div>
  );
}
