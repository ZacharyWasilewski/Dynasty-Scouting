    ...
    <section className="border-y border-border bg-void py-14 sm:py-16">
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
          <div>
            <span className="font-headline text-sm uppercase text-accent">Validation story</span>
            ...
            <div className="mt-6 grid grid-cols-2 gap-px border border-border bg-border">
              <div className="bg-surface p-4"><Target className="h-4 w-4 text-accent" /><p className="mt-3 font-headline text-3xl text-ink">{scored.length}</p><p className="mt-1 text-[10px] text-ink-tertiary">Resolved grades</p></div>
              <div className="bg-surface p-4"><ShieldCheck className="h-4 w-4 text-accent" /><p className="mt-3 font-headline text-3xl text-ink">{topTierHitRate === undefined ? "—" : `${Math.round(topTierHitRate)}%`}</p><p className="mt-1 text-[10px] text-ink-tertiary">Elite+ hit rate</p></div>
            </div>
            <InPageAnchor targetId="performance" extraOffsetId="analytics-quick-nav" className="mt-6 inline-flex items-center gap-1.5 text-[10px] text-accent hover:underline">See the full backtest <ArrowRight className="h-3.5 w-3.5" /></InPageAnchor>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            {stories.map(({ eyebrow, player, score, tone, copy }) => {
              ...
              return <Link key={`${eyebrow}-${player.id}`} href={playerHref(player.id, leagueFormatFromSelection(selection))} className="group border border-border bg-surface p-5 transition-colors hover:border-accent/50 hover:bg-surface-raised">
                <div className="flex items-center justify-between gap-3"><span className="text-[10px] text-ink-tertiary">{eyebrow}</span><Icon className={`h-4 w-4 ${tone === "negative" ? "text-faller" : tone === "positive" ? "text-riser" : "text-accent"}`} /></div>
                ...
                <div className="mt-5 flex items-center justify-between border-t border-border pt-3"><span className="text-xs font-medium text-ink">{outcomeLabel(player)}</span>{tier && <span className="text-[10px]" style={{ color: getTierColor(tier) }}>{tier}</span>}</div>
              </Link>;
            })}
          </div>
        </div>
