...
export function CalibrationCurveChart({ prospects }: { prospects: Prospect[] }) {
  ...
  const [position, setPosition] = useState<PositionCalibration["position"]>("QB");
  ...
  return (
    <div>
      ...
      <p className="mb-5 font-mono text-[9px] uppercase tracking-widest text-ink-tertiary">
        Same for 1QB and Superflex — this curve doesn&apos;t change by league format
      </p>
          ...
          <p className="mt-3 max-w-xl text-xs leading-relaxed text-ink-tertiary">
            {mode === "weighted" ? (
              <>
                ...
                weigh outcome quality instead of a plain hit-or-miss —
                a SuperStar counts for more than a bare hit, and a
                total bust drags it to zero. Each gray dot is
                reality — the true average outcome for actual{" "}
                {position}s who landed in that 10-point range, with
                bigger dots meaning more players behind that number.
                When a dot sits right on the line, the score called
                that group correctly.
              </>
            ) : (
              <>
                The blue line is DD Score&apos;s actual prediction: at
                each score, how often a player like that turns into a
                real fantasy asset. Each gray dot is reality — the
                true hit rate for actual {position}s who landed in
                that 10-point range, with bigger dots meaning more
                players behind that number. When a dot sits right on
                the line, the score called that group correctly. When
                dots drift away from the line, that&apos;s a spot
                where this position&apos;s score has run a little hot
                or cold historically.
              </>
            )}
          </p>
        </>
