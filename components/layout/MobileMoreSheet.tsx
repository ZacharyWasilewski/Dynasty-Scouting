...
//
// Mock Draft removed from this list — it's one of the bottom tab
// bar's five always-visible destinations, so it was reachable two
// ways at once for no reason. Same logic removed Players from its
// own former "Database" section below (also a tab bar destination);
// Classes moved into Research instead of being left alone in a
// section that would otherwise have had exactly one item.
...
export function MobileMoreSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { user, logout, loading: authLoading, error: authError, refresh } = useAuth();
  ...
  return (
    <DialogSurface onClose={() => onClose()} aria-label="More navigation"  className="fixed inset-0 z-[100] lg:hidden">
      <button
      ...
      >
        <div className="sticky top-0 flex items-center justify-between border-b border-border bg-surface px-4 py-3">
          <span className="font-headline text-sm uppercase text-ink-tertiary">Menu</span>
        ...
        <div className="border-b border-border py-2">
          <p className="px-4 py-2 text-[10px] text-ink-tertiary">Tools</p>
          {TOOL_LINKS.map((link) => (
            <Link
              key={link.label}
              href={link.href}
        ...
        <div className="border-b border-border py-2">
          <p className="px-4 py-2 text-[10px] text-ink-tertiary">Research</p>
        ...
        <div className="py-2">
          {authLoading ? null : authError ? (
            <p role="alert" className="px-4 py-3 text-sm text-faller">{authError} <button onClick={() => void refresh()} className="text-accent">Retry</button></p>
          ) : user ? (
            <>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  catch { setLogoutError("Couldn’t log out. Please try again."); }
              ...
              {logoutError && <p role="alert" className="px-4 py-2 text-xs text-faller">{logoutError}</p>}
            </>
          ) : (
            <>
              <Link
