...
const FOOTER_COLUMNS = [
  ...
  { title: "Company", links: [{ label: "About", href: "/about" }, { label: "Privacy & Data", href: "/privacy" }] },
];
...
function DataFreshness({ lastDataRefresh }: { lastDataRefresh: string | null }) {
  ...
  const time = new Date(lastDataRefresh).toLocaleTimeString("en-US", { timeZone: "America/Detroit", timeZoneName: "short", hour: "numeric", minute: "2-digit" });
  return (
        ...
        <div className="lg:flex lg:items-start lg:gap-24">
          ...
          <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-4 lg:mt-0 lg:flex lg:gap-16">
            {FOOTER_COLUMNS.map((col) => (
              <div key={col.title}>
                <h3 className="font-headline text-sm uppercase text-ink-tertiary">{col.title}</h3>
