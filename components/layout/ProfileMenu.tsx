...
import { User, LayoutGrid, Bookmark, ListOrdered, ClipboardList, Link2, LogOut, LogIn, UserPlus } from "@/components/ui/SiteIcons";
...
export function ProfileMenu({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const { user, logout, loading, error: authError, refresh } = useAuth();
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open &&
        (loading || authError ? (
          <div role="status" className="absolute right-0 top-full z-50 mt-2 w-56 border border-border bg-surface p-4 text-sm text-ink">
            {loading ? "Checking sign-in…" : <>{authError}<button className="mt-2 block text-accent" onClick={() => void refresh()}>Retry</button></>}
          </div>
        ) : user ? (
          <div
            role="menu"
          ...
          >
            ...
            <button
              ...
              onClick={async () => {
                setLoggingOut(true);
                setLogoutError(null);
                try { await logout(); onOpenChange(false); }
