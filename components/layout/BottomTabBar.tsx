...
export function BottomTabBar() {
  ...
  return (
    <>
      <nav
        aria-label="Primary"
        // Solid bg-void, no opacity modifier — same real bug already
        // diagnosed and fixed on the desktop Navbar (see its own
        // comment): `void` is registered in tailwind.config.ts as a
        // bare `var(--color-void)` reference, not the
        // `rgb(var(...) / <alpha-value>)` triplet Tailwind's opacity
        // modifiers need, so `bg-void/95` silently failed to apply
        // real opacity. Confirmed directly from a screenshot: this
        // bar was letting dark section backgrounds bleed through
        // behind it, making its own icons/labels unreadable exactly
        // where it matters most (scrolled over a dark section). A
        // sticky, always-visible tab bar's legibility is worth more
        // than the blur effect that was never actually working.
        className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-void lg:hidden"
