...
export function Navbar({ featuredClassYear }: { featuredClassYear?: string }) {
  ...
  //
  // Keep document height constant: the header slides over a fixed-height
  // document spacer instead of changing layout during a scroll gesture.
  ...
  useEffect(() => {
    ...
    const changeHidden = (value: boolean) => {
      setHidden(value);
      lockedUntil = Date.now() + 350;
    };
    const unregister = registerHeaderHiddenSetter(changeHidden);
    function handleScroll() {
      // Clamp Safari's overscroll at either end of the document.
      const maxY = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
      const y = Math.max(0, Math.min(window.scrollY, maxY));
      setScrolled(y > 8);
      if (window.innerWidth >= 1024) { lastY = y; return; }
      if (isProgrammaticScroll()) {
        refreshProgrammaticScroll();
        lastY = y;
        clearTimeout(settleTimeout);
        settleTimeout = setTimeout(() => {
          // Anchor navigation already chose the header state. Only reveal
          // at the document top; don't move the destination after arrival.
          if (window.scrollY <= 4) changeHidden(false);
          lastY = window.scrollY;
        }, 450);
        return;
      }
      if (y <= 4) { setHidden(false); lastY = y; return; }
      if (profileOpenRef.current || pressedControl || Date.now() < lockedUntil) {
        lastY = y;
        return;
      }
      const delta = y - lastY;
      if (Math.abs(delta) > 24) {
        changeHidden(delta > 0 && y > 64);
        lastY = y;
      }
    }
    ...
    function handlePointerDown(event: PointerEvent) {
      pressedControl = event.target instanceof Element && !!event.target.closest("button, a, input, select, [role='tab']");
    }
    function handlePointerUp() {
      if (pressedControl) lockedUntil = Date.now() + 350;
  ...
  useEffect(() => {
    ...
    headerRef.current?.toggleAttribute("inert", hidden);
  }, [hidden]);
  ...
  return (
    ...
    <header
      ...
      className={cn(
        "fixed inset-x-0 top-0 z-50 h-16 border-b bg-void transition-[transform,box-shadow,border-color] duration-300 motion-reduce:transition-none lg:!translate-y-0",
        hidden ? "-translate-y-full pointer-events-none border-transparent" : cn("translate-y-0", scrolled ? "border-border shadow-[0_8px_24px_-16px_rgba(0,0,0,0.8)]" : "border-transparent")
      )}
    >
      <Container>
