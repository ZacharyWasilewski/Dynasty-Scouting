...
export function SubScoreRadar({
  ...
  return (
    // width/height are intentionally CSS-driven (w-full via className,
    // max-width capped at the intended design size), not the raw size
    // prop — a fixed pixel width here overflowed at exactly 320px, the
    // narrowest commonly supported viewport: this Container's own
    // px-6 padding (48px total) left 272px of content width, 8px
    // narrower than a hardcoded 280px chart. viewBox alone still
    // drives every coordinate below, so this only changes how large
    // that coordinate system renders — none of the point/ring math
    // changes.
    <svg
