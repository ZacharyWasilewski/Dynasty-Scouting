...
import { User, CheckCircle2, XCircle, MinusCircle, ArrowLeft, BookOpen, ArrowRight } from "@/components/ui/SiteIcons";
...
import { useProfileTabs } from "@/components/profile/ProfileTabsContext";
import { ProfileTabPanel } from "@/components/profile/ProfileTabPanel";
import { InPageAnchor } from "@/components/ui/InPageAnchor";
import { forceHeaderCollapse } from "@/lib/programmaticScroll";
...
export function ProfileHeader({
  ...
  /** Progression/Comps/Draft panel content — rendered inside this
   *  same card, after the Overview panel, specifically so the sticky
   *  tab bar's containing block spans the full height of all four
   *  panels combined, not just ProfileHeader's own section. Without
   *  this, the tab bar only stayed pinned while scrolling through
   *  Overview (whose content lives inside this component) and
   *  disappeared entirely once scrolled into Progression/Comps/Draft
   *  — their content used to live in sibling sections rendered by
   *  page.tsx *after* this component's own section had already
   *  closed, which is outside any single element position: sticky
   *  can stay pinned against. */
  children?: ReactNode;
}) {
  ...
  // labeling one.
  const usesRawScore = playerScoreStage(prospect) === "raw";
  ...
  const bioLabel =
    prospect.heightIn || prospect.weightLbs
      ? [
          ...
          .join(" · ")
      : null;
  const displayScore = ddScore ?? (!isDrafted ? applyFormatAdjustment(usesRawScore ? prospect.rawScore : prospect.preDraftScore, prospect.position, selectedFormat) : undefined);
  ...
  const rawSummaryScore = applyFormatAdjustment(prospect.rawScore, prospect.position, selectedFormat);
  const summaryRows = [
    ["Raw Score", rawSummaryScore],
    ["Pre-Draft Score", !isDrafted && !usesRawScore ? displayScore : (isDrafted ? applyFormatAdjustment(prospect.preDraftScore, prospect.position, selectedFormat) : undefined)],
    ["O.I.S. Score", isDrafted ? applyFormatAdjustment(prospect.opportunityScore, prospect.position, selectedFormat) : undefined],
    ...
    <section className="border-b border-border bg-grid-columns py-3 sm:py-5 lg:py-7">
      <div className="mx-auto w-full max-w-[1840px] px-3 sm:px-5 lg:px-8 xl:px-10">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <Link
            href="/players"
            className="flex w-fit items-center gap-1.5 text-[10px] text-ink-tertiary transition-colors hover:text-accent"
          >
        ...
        {/* overflow-hidden removed — it was silently breaking the tab
            bar's `sticky` positioning below. Any ancestor with an
            overflow value other than visible becomes the containing
            block sticky positions against, so the bar was "sticking"
            relative to this card (which itself scrolls with the
            page) instead of the viewport — it looked like sticky
            wasn't working at all, when actually it was working
            against the wrong box. Safe to drop here: the only content
            that ever needed clipping to this card's rounded corners
            (the player photo) already has its own separate
            overflow-hidden wrapper a few lines down, and everything
            else in the card is inset from the edges by its own
            padding, not flush against them. */}
        <div className="rounded-2xl border border-border bg-surface/80 shadow-[0_16px_50px_rgba(15,23,42,0.06)]">
          ...
          <div className="grid gap-3 p-4 sm:gap-5 sm:p-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(420px,0.95fr)] lg:gap-8 lg:p-7 xl:p-8">
              ...
              <div className="min-w-0 self-start">
                <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-x-2 gap-y-1">
                  ...
                  <div className="min-w-0 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-ink-secondary sm:text-sm">
                    ...
                    {prospect.draftClass && <><span>·</span><Link href={`/classes/${prospect.draftClass}?format=${selectedFormat === "1QB" ? "1qb" : selectedFormat === "1QB_TEP" ? "1qb-tep" : selectedFormat === "SUPERFLEX_TEP" ? "sf-tep" : "sf"}`} className="font-data hover:text-accent">{prospect.draftClass}</Link></>}
                  </div>
                ...
                <div className="mt-2 flex flex-wrap items-end gap-x-3 gap-y-1 sm:mt-3">
                  <div
                    ...
                    style={displayScore === 100 ? { WebkitTextStroke: "1.5px #FACC15", paintOrder: "stroke fill" } : undefined}
                  >
                  ...
                  <div className="pb-1">
                    <div
                      ...
                      style={displayScore === 100 && tier === "Generational" ? { WebkitTextStroke: "1px #FACC15", paintOrder: "stroke fill" } : undefined}
                    >
                    ...
                    <div className="mt-1 text-[10px] text-ink-tertiary">{isDrafted ? "Dynasty Database Score" : usesRawScore ? "Raw Score" : "Pre-Draft Score"}</div>
                  </div>
                </div>
            ...
            <div className="min-w-0 lg:pt-1">
              <div className="grid grid-cols-3 grid-rows-[auto_auto_auto] overflow-hidden rounded-xl border border-border bg-void/25">
                <div className="row-span-3 grid grid-rows-subgrid p-3 sm:p-4">
                  <p className="text-[10px] text-ink-tertiary">Class Rank</p>
                  <p className="mt-1 font-headline text-2xl leading-none tabular-nums text-ink sm:text-3xl">#{classRank?.rank ?? "—"}</p>
                  <p className="mt-1 text-[10px] text-ink-tertiary">{classRank?.rank ?? "—"} of {classRank?.total ?? "—"} scored</p>
                </div>
                <div className="row-span-3 grid grid-rows-subgrid border-l border-border p-3 sm:p-4">
                  <p className="text-[10px] text-ink-tertiary">Position Rank</p>
                ...
                <div className="row-span-3 grid grid-rows-subgrid border-l border-border p-3 sm:p-4">
                  <div className="flex items-center gap-1"><p className="text-[10px] text-ink-tertiary">{tierHistoryLabel(prospect)} Tier Hit Rate</p><TierHitRateInfo stage={tierHistoryLabel(prospect)} position={prospect.position} tier={tier} format={selectedFormat.replace("SUPERFLEX", "Superflex").replace("_TEP", " · TEP")} count={tierHitRate?.count ?? 0} /></div>
                  <p className="mt-1 font-headline text-2xl leading-none tabular-nums sm:text-3xl" title={`Historical players at this position in the same ${tierHistoryLabel(prospect)} score tier and league format. HIT / (HIT + MISS); pushes and unresolved outcomes are excluded. This is a cohort rate, not an individual prediction.`} style={{ color: tier && hasTierHitRate ? getTierColor(tier) : undefined }}>{tier && hasTierHitRate && tierHitRate ? `${Math.round(tierHitRate.hitRate as number)}%` : "—"}</p>
                  <p className="mt-1 text-[10px] text-ink-tertiary">{tier && hasTierHitRate && tierHitRate ? `${Math.round(((tierHitRate.hitRate as number) / 100) * tierHitRate.count)} of ${tierHitRate.count} hit${tierHitRate.count < 10 ? " · Small sample" : ""}` : "No resolved sample"}</p>
                </div>
              </div>
              {(prospect.finish || prospect.hitMiss) && (
                <div className="mt-3 flex items-center justify-between gap-4 rounded-xl border border-border bg-void/20 px-4 py-3">
                  <div><p className="text-[10px] text-ink-tertiary">Career Outcome</p><div className="mt-1.5">{prospect.finish && <Badge tone="neutral">{prospect.finish}</Badge>}</div></div>
                  {/* hitMiss only ever holds HIT or MISS (never RP —
                      that's stored on `finish` instead, e.g. the
                      test fixture finish: "RP"), which is exactly
                      why RP/push outcomes never got their own icon
                      here before: this only checked hitMiss.
                      Reported directly. Matches RankingsTable's own
                      p.finish?.toUpperCase() === "RP" check for the
                      same value, and reuses the exact gold already
                      used for the Upside Shot tier rather than
                      introducing a second, different yellow. */}
                  {prospect.hitMiss ? (
                    <span className={cn("flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-wide", prospect.hitMiss === "HIT" ? "text-riser" : "text-faller")}>{prospect.hitMiss === "HIT" ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}{prospect.hitMiss}</span>
                  ) : prospect.finish?.toUpperCase() === "RP" ? (
                    <span className="flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-wide" style={{ color: "#C99700" }}><MinusCircle className="h-4 w-4" />RP</span>
                  ) : null}
                </div>
              )}
            </div>
          ...
          {/* Sticky once scrolled past, matching the same pattern
              already used for the Analytics quick-nav (top-[var(--nav-offset)]/
              z-40 = right below the main site nav, wherever that
              currently is — see globals.css's --nav-offset comment
              for why this isn't a hardcoded top-16). Solid bg-surface, not
              the bg-void/20 tint used elsewhere on this card — a
              pinned bar needs a guaranteed-opaque background so
              content scrolling underneath it doesn't show through;
              bg-void's opacity modifiers are unreliable (same root
              cause already fixed once on the mobile bottom tab bar).
              Paired with scroll-mt on each panel's target element
              below so clicking a tab scrolls its content to sit just
              below this bar rather than partly hidden behind it. */}
          <div id="profile-tab-bar" className="sticky top-[var(--nav-offset)] z-40 grid grid-cols-4 divide-x divide-border border-b border-border bg-surface text-center transition-[top] duration-300">
            {/* transition-[top] duration-300, matching the header's
                own max-height transition — top: var(--nav-offset)
                previously snapped to its new position the instant the
                variable changed, while the header itself was still
                300ms into growing or shrinking. Reported directly as
                "the player menu moves down instantly... leaves a gap
                in time where there's nothing there" — this bar and
                the header now animate on the same duration, so they
                move together instead of one outrunning the other. */}
            {/* border-b only, not border-y — the main nav above this
                (Navbar.tsx) already grows its own shadow once the page
                scrolls (its own transition-shadow), and that shadow was
                landing directly against this bar's top border once
                pinned, reading as a slightly messy double line right at
                the seam. One clean edge (the bottom, separating the bar
                from the content below it) instead. */}
            {/* Divider lines between cells + slightly darker inactive
                text (ink-tertiary → ink-secondary) — this used to read
                as a single strip of running text rather than four
                separate, tappable regions, and got missed entirely by
                a first-time visitor until they happened to tap it.
                Kept deliberately restrained per feedback: no icon, no
                pill/rounded-button styling, no color change beyond a
                touch more contrast — just enough segmentation that it
                reads as a tab bar rather than a caption. */}
            {([
              ["overview", "Overview", "Overview", "overview"],
              ["progression", "Progression", "Class Progression", "class-progression"],
              ["comps", "Comps", "Similar Prospects", "similar-prospects"],
              ["draft", "Draft", "Draft Projection", "draft-projection"],
            ] as const).map(([id, short, full, targetId]) => (
              <InPageAnchor
                key={targetId}
                targetId={targetId}
                extraOffsetId="profile-tab-bar"
                onBeforeScroll={() => {
                  setActiveTab(id);
                  // Forces the header to collapse before this tab's
                  // scroll starts, not after it lands — see
                  // programmaticScroll.ts's own comment on why that
                  // ordering is what actually matters here. Safe
                  // assumption specifically for this tab bar: every
                  // panel's content is reliably below the fold, so
                  // there's no real case where the header should stay
                  // expanded after tapping one of these.
                  forceHeaderCollapse();
                }}
                aria-current={activeTab === id ? "page" : undefined}
                className={cn(
                  "flex items-center justify-center px-1 py-4 font-mono text-[10px] uppercase tracking-wide text-ink-secondary hover:bg-surface hover:text-accent sm:py-2.5 sm:text-[11px] sm:tracking-widest2",
                  // Just the blue text, no underline/background — the
                  // divider lines between cells already give this bar
                  // its structure; the underline plus background swap
                  // on top of that was reported as looking heavier and
                  // worse than the plain inactive tabs, not better.
          ...
          <ProfileTabPanel id="overview">
            <div id="overview" className="grid gap-4 p-4 pt-4 scroll-mt-[calc(var(--nav-offset)+56px)] sm:p-5 lg:grid-cols-[0.82fr_1.28fr_0.9fr] lg:p-6">
              <div className="rounded-xl border border-border bg-void/15 p-4">
                <div className="mb-3 flex items-center justify-between gap-3"><p className="text-[10px] font-semibold text-ink-secondary">Prospect Profile</p><span className="font-mono text-[8px] text-ink-tertiary">Percentile · 0–100</span></div>
              ...
              <div className="rounded-xl border border-border bg-void/15 p-4">
                <p className="text-[10px] font-semibold text-ink-secondary">Subscores</p>
