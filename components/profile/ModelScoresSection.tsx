...
export function ModelScoresSection({
  ...
  const stageInfo: Record<ProgressStageKey, string> = {
    raw: "How we judge a player based on quantitative college metrics. This score ignores mock draft data, opportunity, and real life draft position.",
    ...
    dd: "The final Dynasty Database Score, which calibrates the core positional grade against historical outcomes.",
  };
  ...
  return (
    <section id="class-progression" className="scroll-mt-[calc(var(--nav-offset)+56px)] border-b border-border bg-surface py-7 sm:py-8">
      {/* Plain div, not this file's own Container — that added a
          second, narrower max-width/padding on top of the one
          ProfileHeader's card already applies now that this section
          renders nested inside it (for the sticky tab bar fix — see
          ProfileHeader's children-prop comment), which was making
          this panel's content sit visibly narrower/more inset than
          its sibling panels in the same card. p-4/p-5/p-7/p-8 matches
          the padding every other in-card block uses (e.g. the top
          info grid) — not px-3/px-5/px-8/px-10, which is the outer
          page-level inset around the whole card, a different, larger
          scale that would leave this panel's content sitting closer
          to the card's edge than its siblings. */}
        ...
        {progression.length >= 1 ? (
          <div className="mt-5 overflow-hidden rounded-xl border border-border bg-void/30 p-4 sm:p-5">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div className="font-headline text-sm uppercase text-ink-tertiary">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-border pt-4 sm:flex-nowrap">
              <div className="font-headline text-sm uppercase text-ink-tertiary">Opportunity independent</div>
