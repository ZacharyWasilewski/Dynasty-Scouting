...
export function ScoreRing({
  ...
  const fontSizeClass = size < 100 ? (isText ? "text-[9px]" : "text-lg") : isText ? "text-xl" : "text-3xl";
  // Text-mode values (e.g. "DEPTH") have no natural width limit the
  // way a short number does, and at small ring sizes a word that long
  // doesn't fit on one line inside the circle at any reasonable font
  // size — it was overflowing past the ring's edge. Reported directly,
  // with a photo. Constrained to roughly the ring's own inscribed
  // square (a circle's safe rectangular content area) and allowed to
  // wrap, the same way the label below the ring already wraps at
  // small sizes instead of trying to force everything onto one line.
  const textValueMaxWidth = isText && size < 100 ? Math.round(size * 0.62) : undefined;
  const labelSizeClass = size < 100 ? "max-w-[64px] text-[8px] leading-tight" : "text-[10px]";
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span
            className={cn(
              "font-mono font-semibold tabular-nums transition-opacity duration-500 text-center leading-tight",
              fontSizeClass,
              hasContent ? "text-ink" : "text-ink-tertiary",
              mounted ? "opacity-100" : "opacity-0"
            )}
            style={textValueMaxWidth ? { maxWidth: textValueMaxWidth } : undefined}
          >
      ...
      <div className="flex min-w-0 items-center justify-center gap-1">
        <span className={cn("text-center text-ink-tertiary", size < 100 ? "whitespace-normal" : "whitespace-nowrap", labelSizeClass)}>
          {label}
        </span>
      ...
      {hasInfo && showPopup && typeof document !== "undefined" && createPortal(
        <DialogSurface onClose={() => setShowPopup(false)}  className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={`About ${label}`}>
          <div className="absolute inset-0 bg-void/80 backdrop-blur-sm" onClick={() => setShowPopup(false)} />
          <div className="relative z-10 max-h-[85dvh] overflow-y-auto w-full max-w-sm border border-border-strong bg-surface p-6 shadow-xl">
            <button
