...
export function PlayerComparison({
  ...
  const featuredTier = currentResolved
    ...
    : getTierForScore(scoreAtStage(featured, sharedScoreStage(current, featured), format));
    ...
    <div>
        ...
        >
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <span className="font-headline text-sm uppercase text-accent">Closest Match</span>
          ...
          >
            {scoreAtStage(featured, sharedScoreStage(current, featured), format)?.toFixed(1) ?? "—"}
          </span>
        </button>
      )}
      ...
      {rest.length > 0 && (
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          // stage of the model.
          const score = scoreAtStage(p, sharedScoreStage(current, p), format);
          // Keep the score color on the same comparison basis as the
          // number itself.
          const rowTier = currentResolved ? getDDTier(p, format) : getTierForScore(score);
          return (
            <button
              key={p.id}
      ...
      {comparing && (
        <DialogSurface onClose={() => setComparing(null)} aria-label="Player comparison"  className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-void/80 backdrop-blur-sm" onClick={() => setComparing(null)} />
          <div className="relative z-10 max-h-[85dvh] overflow-y-auto w-full max-w-lg border border-border-strong bg-surface p-6 shadow-xl sm:p-8">
            <button
