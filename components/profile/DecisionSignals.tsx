function ordinal(n: number) { const value = Math.round(n); const mod = value % 100; return `${value}${mod >= 11 && mod <= 13 ? "th" : value % 10 === 1 ? "st" : value % 10 === 2 ? "nd" : value % 10 === 3 ? "rd" : "th"}`; }
  ...
  return (
    <div className="border-t border-border bg-void/15 px-4 py-3 sm:px-5 lg:px-6">
      ...
      <p className="mt-1.5 text-xs leading-relaxed text-ink-tertiary sm:text-sm">
        ...
        {qualitativeLabelForPercentile(strongest.value)}, {ordinal(strongest.value)} percentile).
        {hasRealSpread ? (
          <>
            {" "}Weakest: <span className="font-semibold text-ink">{weakest.label}</span> (
            {qualitativeLabelForPercentile(weakest.value)}, {ordinal(weakest.value)} percentile).
          </>
        ) : (
          " The measured signals are closely grouped."
        )}
      </p>
    </div>
  );
