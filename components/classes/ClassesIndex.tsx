...
import { getDbCounts, getDataFreshness } from "@/lib/dbMetadata";
...
export function ClassesIndex({ prospects }: { prospects: Prospect[] }) {
  ...
  // Single authoritative source for the two headline numbers below —
  // see lib/dbMetadata.ts for why this replaced a locally-computed
  // total that could silently disagree with the same count shown on
  // Analytics/the homepage.
  const { prospectCount } = getDbCounts(prospects);
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)] lg:items-end">
            <div>
              <div className="flex items-center gap-3 text-accent">
                ...
                <span className="font-headline text-sm uppercase">Draft Classes</span>
              </div>
            ...
            <div className="border-l border-border pl-6 sm:pl-8 lg:pb-1">
              <span className="font-headline text-sm uppercase text-ink-tertiary">The database timeline</span>
              <div className="mt-4 flex items-end gap-8">
                <div>
                  <div className="font-headline text-5xl leading-none text-ink">{years.length}</div>
                  <div className="mt-2 font-headline text-sm uppercase text-ink-tertiary">Classes tracked</div>
                </div>
                <div className="h-10 w-px bg-border" />
                <div>
                  <div className="font-headline text-5xl leading-none text-ink">{prospectCount.toLocaleString()}</div>
                  <div className="mt-2 font-headline text-sm uppercase text-ink-tertiary">Graded prospects</div>
                </div>
              </div>
              <DataFreshness lastUpdated={lastUpdated} className="mt-4 block" />
      ...
      <section className="border-b border-border bg-void">
        <Container className="py-10 sm:py-14">
          <div className="flex items-end justify-between gap-6">
            <div>
              <span className="font-headline text-sm uppercase text-accent">Now in focus</span>
            ...
            <span className="hidden font-headline text-sm uppercase text-ink-tertiary sm:block">Live evaluation timeline</span>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <span className="font-headline text-sm uppercase text-ink-tertiary">{meta.eyebrow}</span>
      ...
      <section>
        <Container className="py-12 sm:py-16">
          <div className="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-16">
            <div>
              <span className="font-headline text-sm uppercase text-ink-tertiary">The full archive</span>
              ...
              {archive.map((year) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <div className="font-headline text-sm uppercase text-ink-secondary">{meta.eyebrow}</div>
                      <div className="mt-1 text-xs text-ink-tertiary sm:text-sm">{meta.label}</div>
                    </div>
                    <span className="hidden font-mono tabular-nums text-[11px] text-ink-tertiary sm:block">{count > 0 ? `${count} prospects` : "Early look"}</span>
