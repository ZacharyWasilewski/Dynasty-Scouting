...
import { SavedDraftBoard, type SavedBoardPick } from "@/components/mockDrafts/SavedDraftBoard";
...
interface SavedDraft {
  id: string;
  classYear: string;
  settings: { fullBoard?: SavedBoardPick[]; teams: number; slot?: number; qbFormat: MockQBFormat; teFormat: MockTEFormat };
  picks: SavedPick[];
...
export function SavedMockDraftDetail({ id }: { id: string }) {
  ...
  const [draft, setDraft] = useState<SavedDraft | null | "not-found" | "error">(null);

  useEffect(() => {
    if (!user) return;
    fetch(`/api/mock-drafts/${id}`)
      .then((res) => (res.status === 404 ? { draft: "not-found" } : res.ok ? res.json() : Promise.reject()))
    ...
    <main className="py-10">
      <Container className="max-w-2xl">
        ...
        {authLoading ? (
          ...
          <div className="mt-8 h-24" />
        ) : draft === "error" ? (<p role="alert" className="mt-8 text-sm">Could not load this draft. <button className="underline" onClick={() => window.location.reload()}>Try again</button></p>) : draft === "not-found" ? (
          ...
          <>
            <div className="mt-5 flex items-center justify-between gap-4">
              <div>
                <p className="font-headline text-sm uppercase text-accent">
            ...
            <SavedDraftBoard picks={draft.settings.fullBoard} slot={draft.settings.slot} teams={draft.settings.teams} format={toLeagueFormat(draft.settings.qbFormat, draft.settings.teFormat)} />
            <div className="mt-6 border border-border bg-surface">
              <div className="border-b border-border px-4 py-3">
                <p className="text-[10px] text-ink-tertiary">Your picks</p>
              </div>
