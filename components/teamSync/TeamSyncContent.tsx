...
export function TeamSyncContent() {
    ...
    return (
      <div className="max-w-2xl">
        <div className="mb-6 border border-border bg-surface p-5 sm:p-6">
          <p className="font-headline text-sm uppercase text-accent">Connect Your League</p>
      ...
      <div className="max-w-md">
        {savedTeams && savedTeams.length > 0 && (
          <div className="mb-6">
            <p className="mb-3 font-headline text-sm uppercase text-ink-tertiary">
              Your synced teams
            </p>
            ...
            <p className="mt-3 font-headline text-sm uppercase text-ink-tertiary">
              Sync another league
            </p>
          </div>
        )}
        ...
        {mode === "leagueId" ? (
          <>
            ...
            {loading && (
              <div className="mt-3 flex items-center gap-2 text-xs text-ink-secondary">
                <Loader2 className="h-3.5 w-3.5 animate-spin" /> Loading your league…
              </div>
            )}
          ...
          <>
            ...
            {loading && (
              <div className="mt-3 flex items-center gap-2 text-xs text-ink-secondary">
                <Loader2 className="h-3.5 w-3.5 animate-spin" /> Finding your Sleeper leagues…
              </div>
            )}
            <p className="mt-3 text-xs text-ink-tertiary">
  ...
  if (step.kind === "userLeagues") {
    return (
      <div className="max-w-md">
        ...
        <p className="mb-3 font-headline text-sm uppercase text-ink-tertiary">Pick a league</p>
        <div className="border border-border bg-surface">
          {step.leagues.map((l) => (
            <button
              key={l.leagueId}
  ...
  // Both derived straight from result.needs (the exact same data the
  // "Positional value" grid below already renders per-position) —
  // not a new calculation, just surfacing the two extremes of an
  // existing array right under the grade instead of only inside a
  // 5-column grid further down the page.
  const strongestNeed = [...result.needs].sort((a, b) => b.percentile - a.percentile)[0];
  const weakestNeed = [...result.needs].sort((a, b) => a.percentile - b.percentile)[0];
  const topTargetGroup = result.recommendations[0];
  const topTargets = topTargetGroup?.prospects.slice(0, 3) ?? [];
  ...
  return (
    <div>
      ...
      <div className="mt-6 border border-border bg-surface p-5">
        ...
        {/* The three-second read: what's good, what's not, what to
            do about it — before anything else on the page, matching
            how the grade itself is read first. All three come
            straight from result.needs / result.recommendations, the
            same data the sections below already show in more depth. */}
        <div className="mt-4 grid grid-cols-3 gap-3 border-t border-border pt-4">
          <div>
            <p className="text-[10px] text-riser">Strength</p>
            <p className="mt-1 text-lg font-semibold text-ink">{strongestNeed?.position ?? "—"}</p>
          </div>
          <div>
            <p className="text-[10px] text-faller">Weakest Area</p>
            <p className="mt-1 text-lg font-semibold text-ink">{weakestNeed?.position ?? "—"}</p>
          </div>
          <div>
            <p className="text-[10px] text-accent">Best Opportunity</p>
            <p className="mt-1 text-lg font-semibold text-ink">{topNeeds[0]?.position ?? weakestNeed?.position ?? "—"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 flex items-center gap-3">
        <span className="font-headline text-sm uppercase text-ink-tertiary">The Details</span>
        <span className="h-px flex-1 bg-border" />
      </div>

      <div className="mt-4 border border-border bg-surface p-4">
        <p className="font-headline text-sm uppercase text-ink-tertiary">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-5">
          {result.needs.map((n) => (
            <div key={n.position} className="border border-border-strong bg-surface-raised p-3">
              ...
              {n.needScore > 0 ? (
                <p className="mt-1 text-[10px] text-faller">Need</p>
              ) : (
                <p className="mt-1 text-[10px] text-riser">Stocked</p>
              )}
            </div>
          ))}
          <div className="col-span-2 border border-border-strong bg-surface-raised p-3 sm:col-span-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-headline text-sm uppercase text-ink-tertiary">Picks</span>
            </div>
            <p className="mt-1.5 text-lg font-semibold text-ink">
            ...
            {result.pickPercentile < 50 ? (
              <p className="mt-1 text-[10px] text-faller">Below avg</p>
            ) : (
              <p className="mt-1 text-[10px] text-riser">Above avg</p>
            )}
          </div>
        </div>
      </div>
      ...
      {result.recommendations[0]?.prospects[0] && (() => {
        ...
        return (
          <div className="mt-6 border border-accent/30 bg-accent/5 p-5 sm:p-6">
            <p className="font-headline text-sm uppercase text-accent">Draft Decision</p>
            ...
            <div className="mt-4 flex items-center justify-between gap-4">
              ...
              <div className="shrink-0 text-right"><p className="font-headline text-3xl text-accent">{score !== undefined ? score.toFixed(1) : "—"}</p><p className="text-[10px] text-ink-tertiary">DD Score</p></div>
            </div>
            ...
            {topTargets.length > 1 && (
              <div className="mt-4 border-t border-accent/20 pt-4">
                <p className="text-[10px] text-ink-tertiary">Top targets</p>
                <ol className="mt-2 space-y-1.5">
                  {topTargets.map((p, i) => {
                    const targetScore = getScoreForFormat(p, result.format, "STANDARD");
                    return (
                      <li key={p.id} className="flex items-center justify-between gap-3 text-sm">
                        <span className="min-w-0 truncate text-ink"><span className="text-ink-tertiary">{i + 1}.</span> {p.name}</span>
                        <span className="shrink-0 font-mono text-xs text-ink-tertiary">DD Score {targetScore !== undefined ? targetScore.toFixed(1) : "—"}</span>
                      </li>
                    );
                  })}
                </ol>
      ...
      {result.moversAtNeeds.length > 0 && (
        <MobileAccordionSection title="Movers at your needs">
      ...
      {topNeeds.length === 0 ? (
        ...
        <MobileAccordionSection title="Full recommendations, by position">
