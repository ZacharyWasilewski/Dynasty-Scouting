...
import { playerHref, formatFromParam } from "@/lib/playerLinks";
...
export function CommandPalette() {
  ...
  useEffect(() => { if (urlFormat) setFormat(formatFromParam(urlFormat)); }, [urlFormat]);
  ...
  function goTo(prospect: SearchProspect) {
    setOpen(false);
    router.push(playerHref(prospect.id, format));
  }
  ...
  return (
    <DialogSurface onClose={() => setOpen(false)} aria-label="Search prospects"  className="fixed inset-0 z-[100] flex items-start justify-center px-4 pt-24 sm:pt-32">
      {/* backdrop */}
      <div
      ...
      {/* panel */}
      <div className="relative z-10 max-h-[85dvh] overflow-y-auto w-full max-w-xl animate-fade-in-up border border-border-strong bg-surface shadow-2xl [animation-duration:200ms]">
