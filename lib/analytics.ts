...
export const DEFAULT_FORMAT: LeagueFormatSelection = { qbFormat: "SF", tepFormat: "STANDARD" };
...
/** The reverse of selectionFromLeagueFormat — needed wherever code
 *  holds a LeagueFormatSelection but needs to call a function that
 *  takes the string-based LeagueFormat instead (e.g.
 *  getDisplayedPreDraftScore, which lives in lib/prospects.ts and
 *  uses the string type everywhere else on the site does). */
export function leagueFormatFromSelection(selection: LeagueFormatSelection): LeagueFormat {
  if (selection.qbFormat === "SF") {
    return selection.tepFormat === "TEP" ? "SUPERFLEX_TEP" : "SUPERFLEX";
  }
  return selection.tepFormat === "TEP" ? "1QB_TEP" : "1QB";
}
...
function displayedTierForProspect(p: Prospect, format: LeagueFormatSelection): Tier | undefined {
  ...
  const score = getDisplayedPreDraftScore(p, leagueFormatFromSelection(format));
...
export function computeHitRateByPositionAndTier(
  ...
  tiers: string[],
  stage?: "dd" | "preDraft" | "raw"
): TierHitRateDatum[] {
  return tiers.map((tier) => {
    const inTier = prospects.filter((p) => {
      if (p.position !== position) return false;
      if (!stage) return displayedTierForProspect(p, format) === tier;
      // Earlier-stage history uses historical players' actual earlier scores,
      // never their final DD tier or a fallback to a different score stage.
      if (stage !== "dd" && !Number.isFinite(stage === "raw" ? p.rawScore : p.preDraftScore)) return false;
      const score = stage === "dd"
        ? (p.hasDraftData === true ? ddScoreForFormat(p, format) : undefined)
        : applyFormatAdjustment(stage === "raw" ? p.rawScore : p.preDraftScore, p.position, leagueFormatFromSelection(format));
      if (!Number.isFinite(score)) return false;
      const stageTier = stage === "dd" ? tierForFormat(p, format) : getTierForScore(score as number);
      return stageTier === tier;
    });
    const resolved = inTier.filter((p) => p.hitMiss === "HIT" || p.hitMiss === "MISS");
    const hits = resolved.filter((p) => p.hitMiss === "HIT").length;
    const { avg, count: valueCount } = meanWeight(inTier);
...
export const CAPITAL_TIERS: CapitalTierDefinition[] = [
  { label: "Picks 1–32", min: 1, max: 32 },
  { label: "Picks 33–64", min: 33, max: 64 },
  { label: "Picks 65–100", min: 65, max: 100 },
  { label: "Picks 101–180", min: 101, max: 180 },
  { label: "Picks 181+", min: 181, max: Infinity },
];
...
export function computeCapitalVsModelHitRates(
  ...
  const cohort = prospects.filter(
    (p) => Number.isFinite(p.adp) && (p.adp as number) >= 1 && ddScoreForFormat(p, format) !== undefined && (p.hitMiss === "HIT" || p.hitMiss === "MISS")
  );
...
/**
 * Thin wrapper around classCycle.ts's getTrackedClassYears — this
 * used to be its own independent reimplementation of the identical
 * "unique draft-class years since 2015" logic, which is exactly the
 * kind of duplicated logic that can silently drift (e.g. if the
 * minYear cutoff ever changed in one copy and not the other).
 * Preserves this function's original descending sort order (newest
 * class first) for existing callers — getTrackedClassYears itself
 * sorts ascending, so reversed here rather than changing its
 * contract for its own callers.
 */
export function draftClassesInData(prospects: Prospect[]): string[] {
  return getTrackedClassYears(prospects).slice().reverse();
}
