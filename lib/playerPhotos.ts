...
interface SleeperPlayer {
  ...
  position?: string;
  team?: string;
  // Sleeper's public API returns both as plain numeric strings (e.g.
  // height: "74" meaning 74 inches, weight: "215" meaning 215 lbs) —
  // not pre-formatted display strings like ESPN's displayHeight.
  height?: string;
  weight?: string;
}

/** A player's height/weight, parsed from whichever source (Sleeper
 *  for drafted players, ESPN for devy) has data for them. Undefined
 *  fields simply mean that source didn't have a value — never a
...
let inFlight: Promise<{ photoIndex: Map<string, string>; byIdIndex: Map<string, RosterPlayerInfo>; bioIndex: Map<string, PlayerBio> }> | null = null;
...
async function fetchPhotoIndex(): Promise<{ photoIndex: Map<string, string>; byIdIndex: Map<string, RosterPlayerInfo>; bioIndex: Map<string, PlayerBio> }> {
  const index = new Map<string, string>();
  const byId = new Map<string, RosterPlayerInfo>();
  // Built from this exact same fetch/loop, not a second request —
  // height/weight are already present on every player object in the
  // same ~5MB response already being fetched for photos; there was
  // never a reason to scrape a separate source for this.
  const bio = new Map<string, PlayerBio>();
  try {
    ...
    if (!res.ok) {
      ...
      return { photoIndex: index, byIdIndex: byId, bioIndex: bio };
    }
    ...
    // Tracks which key currently has a team-having (active/rostered)
    // player behind it, so a later collision with an inactive player
    // never overwrites a good match — see the collision-handling
    // comment below for why this exists at all.
    const hasActiveTeam = new Map<string, boolean>();
    for (const player of Object.values(data)) {
      ...
      const isActive = Boolean(player.team);

      // Two different Sleeper players can share the exact same
      // name+position (a real, if uncommon, collision — e.g. a
      // retired player still present in Sleeper's directory under
      // the same name as a current one). This used to just overwrite
      // on every match, meaning whichever player happened to come
      // LAST in Sleeper's own API response won — and since that
      // response's ordering isn't guaranteed stable across separate
      // fetches, the same name collision could resolve to a
      // different player's photo depending on exactly when the index
      // was last rebuilt, which is exactly what "the photo changed
      // between two different page loads" looks like from the
      // outside. Deterministic instead: prefer whichever entry has a
      // team (an active, actually-rostered player) over one that
      // doesn't, regardless of which was encountered first or last.
      const exactKey = indexKey(name, player.position);
      const currentIsActive = hasActiveTeam.get(exactKey);
      if (currentIsActive === undefined || (isActive && !currentIsActive)) {
        index.set(exactKey, url);
        hasActiveTeam.set(exactKey, isActive);

        // Same collision-preference logic as the photo URL above
        // (prefer the active-team player on a name collision) — reuses
        // the exact same exactKey and currentIsActive/isActive check
        // rather than a second, separately-maintained comparison.
        const heightIn = player.height ? Number(player.height) : undefined;
        const weightLbs = player.weight ? Number(player.weight) : undefined;
        if (Number.isFinite(heightIn) || Number.isFinite(weightLbs)) {
          const parsedBio: PlayerBio = {
            heightIn: Number.isFinite(heightIn) ? heightIn : undefined,
            weightLbs: Number.isFinite(weightLbs) ? weightLbs : undefined,
          };
          bio.set(exactKey, parsedBio);
          // Same name-only fallback reasoning as the photo index just
          // below — a position mismatch between this app's sheet and
          // Sleeper's own database shouldn't mean losing bio data too.
          const nk = nameOnlyKey(name);
          if (!bio.has(nk)) bio.set(nk, parsedBio);
        }
      }
...
/**
 * Height/weight for drafted players, keyed the same way as the photo
 * index (name+position, see indexKey) — same fetch, same cache, same
 * collision handling; not a separate lookup built from anything new.
 */
export async function getSleeperBioIndexIfReady(): Promise<Map<string, PlayerBio>> {
  const result = await ensureFetched();
  return result.bioIndex;
}

/**
 * Single-flight fetch + cache for all three indexes above. The key
 ...
 */
async function ensureFetched(): Promise<{ photoIndex: Map<string, string>; byIdIndex: Map<string, RosterPlayerInfo>; bioIndex: Map<string, PlayerBio> }> {
  if (photoIndex && byIdIndex && bioIndex && photoIndexExpires > Date.now()) {
    return { photoIndex, byIdIndex, bioIndex };
  }
  ...
  if (!inFlight) {
    inFlight = fetchPhotoIndex()
      .then((result) => {
        ...
        return { photoIndex: photoIndex ?? new Map(), byIdIndex: byIdIndex ?? new Map(), bioIndex: bioIndex ?? new Map() };
      })
      .finally(() => {
        inFlight = null;
      });
  ...
  if (wasNeverFetched) {
    return withTimeout(inFlight, COLD_START_TIMEOUT_MS, { photoIndex: new Map(), byIdIndex: new Map(), bioIndex: new Map() });
  }
...
/** Same lookup strategy as lookupPlayerPhoto, for the bio index. */
export function lookupPlayerBio(
  index: Map<string, PlayerBio>,
  name: string,
  position: string
): PlayerBio | undefined {
  return index.get(indexKey(name, position)) ?? index.get(nameOnlyKey(name));
