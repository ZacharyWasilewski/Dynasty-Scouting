...
export function reportStatus(source: string, status: SourceStatus, message: string, lastSuccessAt?: string): void {
  const prev = store.get(source);
  store.set(source, {
    status,
    message,
    lastSuccessAt: status === "ok" ? (lastSuccessAt ?? new Date().toISOString()) : prev?.lastSuccessAt ?? null,
...
export function reportHealthEvent(message: string, lastSuccessAt?: string): void {
