...
export function getCommunityFormatKey(qb: MockQBFormat, te: MockTEFormat): CommunityFormatKey {
  ...
  // TEP is not a separate source board, so the Community layer applies the
...
export function getCommunityFormatLabel(qb: MockQBFormat, te: MockTEFormat): string {
  ...
  const teLabel = te === "TEP" ? "TEP" : "Standard";
 ...
 * case the engine falls back to the sheet's Pre-Draft Score.
 *
 * Real, confirmed bug this fixes: this used to describe Pre-Draft
 * Score as "a single cross-format number" — true when written, no
 * longer true. Pre-Draft Score now gets the same QB/TE format
 * adjustment DD Score always has (see lib/formatAdjustment.ts). This
 * fallback wasn't applying it, which meant the mock draft's own
 * selection/ranking logic silently used the wrong, unadjusted score
 * for any devy prospect whenever the format toggle changed, even
 * though drafted-player scores right next to them in the same board
 * did respond.
 */
export function mockLeagueFormat(qb: MockQBFormat, te: MockTEFormat): LeagueFormat {
...
export function getScoreForFormat(p: Prospect, qb: MockQBFormat, te: MockTEFormat): number | undefined {
  const adjustedPreDraft = applyFormatAdjustment(p.preDraftScore, p.position, mockLeagueFormat(qb, te));
  if (qb === "SUPERFLEX" && te === "TEP") return p.ddScoreSuperflexTEP ?? p.ddScoreSuperflex ?? adjustedPreDraft;
  if (qb === "SUPERFLEX") return p.ddScoreSuperflex ?? adjustedPreDraft;
  if (te === "TEP") return p.ddScore1QBTEP ?? p.ddScore1QB ?? adjustedPreDraft;
  return p.ddScore1QB ?? adjustedPreDraft;
}
...
export function getTierForFormat(p: Prospect, qb: MockQBFormat, te: MockTEFormat): Tier | undefined {
  const tier =
    ...
    p.tier1QB;
  // Same Pre-Draft Score fallback and fix as getScoreForFormat above.
  const adjustedPreDraft = applyFormatAdjustment(p.preDraftScore, p.position, mockLeagueFormat(qb, te));
  return tier ?? getTierForScore(adjustedPreDraft);
}
...
/** Compare each selected score with the best score still on the board at that pick. */
export function computeDDValueCaptured(
  rows: { player: Prospect; pick: Pick<MockPick, "overall"> }[],
  prospects: Prospect[], picks: Pick<MockPick, "overall" | "playerId">[],
  qb: MockQBFormat, te: MockTEFormat
): number {
  let actual = 0;
  let optimal = 0;
  for (const row of rows) {
    actual += getScoreForFormat(row.player, qb, te) ?? 0;
    const available = prospects.filter((p) => {
      const selection = picks.find((pick) => pick.playerId === p.id);
      // The current selection was available when this pick began.
      return !selection || selection.overall >= row.pick.overall;
    });
    optimal += Math.max(0, ...available.map((p) => getScoreForFormat(p, qb, te) ?? 0));
  }
