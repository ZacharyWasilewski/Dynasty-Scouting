...
async function refreshWithSharedLock(force = false): Promise<SheetData> {
  ...
  return withDbClient(async (client) => {
    ...
    if (!locked) {
      if (force) throw new Error("Another server is refreshing the data. The previous snapshot is retained; try again shortly.");
    ...
    try {
      ...
      if (!force && existing && existing.expires > Date.now()) {
...
/** Manual refreshes share one attempt and only succeed after a live fetch. */
let manualRefresh: Promise<{ prospectCount: number; version: number; refreshedAt: string }> | null = null;
export function forceSheetRefresh(): Promise<{ prospectCount: number; version: number; refreshedAt: string }> {
  if (manualRefresh) return manualRefresh;
  manualRefresh = (async () => {
    // Let any automatic attempt finish before bypassing the shared-store TTL.
    if (inFlight) await inFlight.catch(() => {});
    await startSheetRefresh(true);
    ...
    return { prospectCount: cache.data.prospects.length, version: cache.version,
      refreshedAt: new Date(cache.expires - REVALIDATE_SECONDS * 1000).toISOString() };
  })();
  manualRefresh.finally(() => { manualRefresh = null; }).catch(() => {});
  return manualRefresh;
}

function startSheetRefresh(force = false): Promise<SheetData> {
  ...
  inFlight = (async () => {
    try {
      ...
      if (HAS_SHARED_SNAPSHOT_STORE) {
        return await refreshWithSharedLock(force);
      }
      return await refreshLocally();
    } catch (err) {
      console.error("[sheet-data] Live refresh failed; keeping last known-good data:", err);
      if (force) {
        reportStatus("google-sheet", cache ? "stale" : "error", "Manual refresh failed — previous snapshot retained");
        throw err;
      }
      if (cache) {
...
export async function getSheetSnapshot(): Promise<Readonly<Snapshot>> {
  ...
  if (shared && (!cache || shared.version > cache.version)) {
    ...
    if (latest) {
      promoteSnapshot(latest, "shared");
      reportStatus("google-sheet", latest.expires > now ? "ok" : "stale",
        `${latest.data.prospects.length} prospects loaded from shared snapshot v${latest.version}`,
        new Date(latest.expires - REVALIDATE_SECONDS * 1000).toISOString());
    }
  }
  ...
  if (sharedSnapshot) {
    ...
    reportStatus("google-sheet", sharedSnapshot.expires > now ? "ok" : "stale", `${sharedSnapshot.data.prospects.length} prospects loaded from shared snapshot v${sharedSnapshot.version}`, new Date(sharedSnapshot.expires - REVALIDATE_SECONDS * 1000).toISOString());
