...
export function checkRateLimit(
  ...
  sweep();
  // IDs and years share a route budget; query strings never create new budgets.
  const path = new URL(request.url).pathname.replace(
    /^(\/api\/(?:mock-drafts|board\/opportunity|board))\/[^/]+/, "$1/:id"
  );
  const ip = `${request.method}:${path}:${getClientIp(request)}`;
  const now = Date.now();
  const existing = buckets.get(ip);

  if (!existing || now >= existing.expiresAt) {
    buckets.set(ip, { count: 1, windowStart: now, expiresAt: now + windowMs });
