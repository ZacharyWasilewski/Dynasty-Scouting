...
export async function recordEvent(eventType: string, path: string | null, userId: string | null): Promise<void> {
  try {
    ...
    if (Math.random() < 1 / 500) {
      await query(`DELETE FROM usage_events WHERE event_type = 'page_view' AND created_at < now() - interval '90 days'`);
    }
  } catch (err) {
...
export async function getUsageSummary(days: number, excludeUserId?: string): Promise<{
  ...
  try {
    ...
    const [topPathsRows, eventCountRows, activeUsersRows, totalRows] = await Promise.all([
      ...
      query<{ count: string }>(
        `SELECT COUNT(*) as count FROM usage_events WHERE event_type = 'page_view' AND created_at > now() - ($1 || ' days')::interval ${exclude}`,
        params
      ),
    ]);
    ...
    return { topPaths: [], eventCounts: [], uniqueActiveUsers: 0, totalPageViews: 0, available: false };
  }
}
