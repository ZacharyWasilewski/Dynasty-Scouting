...
export function getOverallScore(p: Prospect): number | undefined {
  ...
  const hasDraftCapital = p.subScores.some((s) => s.label === "Draft Capital");
  if (hasDraftCapital) return p.grade?.overall;
  const hasMock = p.subScores.some((s) => s.label === "Mock");
  if (hasMock) return p.preDraftScore;
  return undefined;
}
...
export function findSimilarProspects(
  ...
  const prospectOverall = prospectResolved ? prospect.grade?.overall : prospect.preDraftScore;
