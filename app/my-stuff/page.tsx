...
import { Bookmark, ListOrdered, ClipboardList, Link2, LogIn, ArrowRight, ShieldCheck } from "@/components/ui/SiteIcons";
...
export default function MyStuffPage() {
  ...
  const { ids: watchlistIds, count: watchlistCount, loadError: watchlistError, hydrated: watchlistHydrated } = useWatchlist();
  ...
  useEffect(() => {
    ...
    fetch("/api/last-seen", {
      ...
      .then((r) => { if (!r.ok) throw new Error("Could not load"); return r.json(); })
  ...
  useEffect(() => {
    ...
    fetch("/api/board")
      ...
      .catch(() => setBoards({ loading: false, error: true, count: 0, detail: "" }));
    ...
    fetch("/api/mock-drafts")
      ...
      .catch(() => setDrafts({ loading: false, error: true, count: 0, detail: "" }));
    ...
    fetch("/api/sleeper?action=saved")
      ...
      .catch(() => setTeams({ loading: false, error: true, count: 0, detail: "", grade: null, moversCount: 0 }));
  }, [user]);
  ...
  const watchlistStatus = watchlistError ? "Could not load — refresh to retry" : watchlistCount === 0
    ? "Nothing saved yet"
  ...
  const teamSyncStatus = teams.error ? "Could not load — refresh to retry" : teams.count === 0
    ? "Not synced yet"
    : [
  ...
  const cards = [
    {
      icon: Bookmark,
      title: "Watchlist",
      what: "A simple list of players you want to keep an eye on. No ranking or ordering, just a quick way to save them.",
      href: "/watchlist",
    ...
    },
    {
      icon: ListOrdered,
      title: "My Big Board",
      what: "Your personal ranking for a draft class. Start with the DD Score order, then move players wherever you want.",
      href: "/board",
      loading: boards.loading,
      status: boards.error ? "Could not load — refresh to retry" : boards.count > 0 ? `${boards.count} board${boards.count === 1 ? "" : "s"}, ${boards.detail}` : "No boards started yet",
    },
    {
      icon: ClipboardList,
      title: "Mock Drafts",
      ...
      status: drafts.error ? "Could not load — refresh to retry" : drafts.count > 0 ? `${drafts.count} draft${drafts.count === 1 ? "" : "s"} completed` : "None completed yet",
    },
    {
      icon: Link2,
      title: "Team Sync",
    ...
    ...(user?.isAdmin
      ? [{
          icon: ShieldCheck,
          title: "Admin Status",
          what: "Private system-health and data-refresh controls for the Dynasty Database administrator.",
          href: "/admin/status",
          loading: false,
          status: "Administrator tools",
        }]
      : []),
  ];
  ...
  return (
    <main>
      <SectionIntro
        ...
        title="My Stuff"
        description="Everything tied to your account, in one place. See what each tool is for and what has changed since you last checked."
       variant="utility" />

      <section className="py-10">
        <Container className="max-w-3xl">
          {previousVisit && (
            <p className="mb-4 font-headline text-sm uppercase text-ink-tertiary">
          ...
          {authLoading ? (
            ...
            <EmptyState
              icon={LogIn}
              title="Log in to see your saved stuff."
              action={{ label: "Log in", href: `/login?redirect=${encodeURIComponent("/my-stuff")}` }}
            />
          ) : (
              ...
              {cards.map((card) => (
                ...
                >
                  ...
                  <div className="mt-4 flex min-w-0 items-center justify-between gap-2">
                    <span className="min-w-0 truncate font-headline text-sm uppercase text-ink-secondary">
