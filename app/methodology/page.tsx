...
import { POSITION_SUBSCORES, METHODOLOGY_POSITIONS, METHODOLOGY_CONTENT, subScoreSlug, subScoreDescription } from "@/lib/methodologySlugs";
  ...
  return (
    <main>
      <section className="border-b border-border bg-grid-columns">
        <Container className="flex flex-col items-start py-12 sm:py-20 lg:py-24">
          <span className="flex h-12 w-12 items-center justify-center border border-accent/40 bg-accent/10 text-accent">
            <Compass className="h-5 w-5" strokeWidth={1.75} />
          </span>
          <span className="mt-6 font-headline text-sm uppercase text-accent">
            Methodology
          </span>
      ...
      {/* Global metric-calculation note — placed first, before any
          per-stage or per-position explanation, since it's a rule
          that applies across all of them ("best qualifying season,
          evaluated independently per metric"), not something specific
          to one position or one score stage. Reuses the exact
          eyebrow/heading/paragraph pattern the "Weighted vs. Standard
          Hit Rates" section below already uses, rather than
          introducing a new card/callout style just for this — kept
          intentionally plain (no border, no background tint) so it
          reads as page content, not a dominant banner. */}
        ...
        <Container>
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-sm leading-relaxed text-ink-secondary">
              Unless otherwise specified, <span className="font-semibold text-ink">all statistical metrics in the Dynasty Database model use the best qualifying season of a player&apos;s college career for each individual metric.</span> This means a player can receive credit for their strongest season in Production, their strongest season in Accuracy, their strongest season in Rushing, and so on. Each metric is evaluated independently to capture the highest level of performance a player demonstrated in college.
            </p>
          </div>
        </Container>
      </section>
      ...
      <section id="weighted-hit-rates" className="scroll-mt-24 border-b border-border py-8 sm:py-14">
        <Container>
          <div className="max-w-2xl">
            <span className="font-headline text-sm uppercase text-accent">
              Analytics
            ...
            <p className="mt-4 text-sm leading-relaxed text-ink-secondary">
              ...
              Standard hit rate is HIT ÷ (HIT + MISS). A 1YS outcome or better
              counts as a hit; Bench and Bust count as misses. RP pushes and
              unresolved outcomes are excluded. This describes historical results
              for a group of prospects, not an individual player&apos;s probability.
            </p>
            <p className="mt-4 text-sm leading-relaxed text-ink-secondary">
          ...
          <div className="mt-6 max-w-md overflow-hidden border border-border">
            <div className="grid grid-cols-[1fr_auto] bg-surface-raised px-4 py-2 text-[10px] text-ink-tertiary">
              <span>Finish</span>
              <span>Weight</span>
            </div>
        ...
        <Container>
          <div className="flex flex-col gap-10 sm:gap-16">
            {METHODOLOGY_POSITIONS.map((position) => {
              ...
              const methodologyContent = METHODOLOGY_CONTENT[position];
              return (
              <div key={position}>
                ...
                <div className="mt-6 flex flex-col divide-y divide-border border-y border-border">
                  {subscores.map((label) => {
                    ...
                    const longForm = methodologyContent?.subscores[label];
                    return (
                        ...
                        {isOpen && (
                          <div className="pb-5">
                            <p className="max-w-2xl text-sm leading-relaxed text-ink-secondary">
                              {longForm?.text ?? subScoreDescription(position, label)}
                            </p>
                            {/* Classifications (QB's Opportunity: QB1,
                                MEN, QB2P, QB2H, DEPTH) render nested
                                directly under their parent sub-score,
                                not as a separate section — they're
                                Opportunity classifications specifically,
                                not general QB classifications, so this
                                keeps that relationship visually obvious
                                rather than implying they're their own
                                independent category. */}
                            {longForm?.classifications && (
                              <dl className="mt-4 max-w-2xl border-l-2 border-border pl-4">
                                {longForm.classifications.map((c) => (
