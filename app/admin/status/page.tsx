...
const STATUS_CLASS: Record<SourceStatus, string> = {
  ...
  // Upside Shot tier and C grade colors had.
  //
  // #8A6608, not the #B7860B originally used here: that first pass
  // computed to only 3.05:1 against the light theme's background,
  // which clears WCAG's 3:1 floor for LARGE text but not the 4.5:1
  // required for normal-size text — and every place this renders is
  // small (11px badges, inline status text). Recomputed properly
  // rather than trusting a value because it was already used
  // elsewhere; #8A6608 clears 4.5:1 with real margin (4.92:1).
  stale: "text-[#8A6608] border-[#8A6608]/30 bg-[#8A6608]/10",
...
export default async function AdminStatusPage() {
  ...
  const sheetSnapshot = await getSheetSnapshot();
  ...
  return (
    <main className="py-10">
      <Container className="max-w-3xl">
        <p className="font-headline text-sm uppercase text-accent">Admin</p>
        ...
        <div className="mt-8 border border-border bg-surface p-4">
          <p className="font-headline text-sm uppercase text-ink-tertiary">Memory (this process)</p>
          <div className="mt-3 grid grid-cols-3 gap-3">
            <div className="border border-border-strong bg-surface-raised p-3">
              <p className="text-[10px] text-ink-tertiary">RSS</p>
              <p className="mt-1 text-lg font-semibold text-ink">{mem.rssMb} MB</p>
            </div>
            <div className="border border-border-strong bg-surface-raised p-3">
              <p className="text-[10px] text-ink-tertiary">Heap used</p>
              <p className="mt-1 text-lg font-semibold text-ink">{mem.heapUsedMb} MB</p>
            </div>
            <div className="border border-border-strong bg-surface-raised p-3">
              <p className="text-[10px] text-ink-tertiary">Heap total</p>
        ...
        <AdminRefreshControl
          initialVersion={sheetSnapshot.version}
          initialProspectCount={sheetSnapshot.data.prospects.length}
        />

        <div className="mt-6 border border-border bg-surface p-4">
          <p className="font-headline text-sm uppercase text-ink-tertiary">Usage, last 7 days</p>
          <p className="mt-1 text-xs text-ink-tertiary">
            Deliberately minimal, aggregate page views and feature-event counts, not a detailed activity log.
          </p>
          {!usage.available ? <p role="alert" className="mt-3 text-sm text-faller">Usage statistics couldn’t be loaded. <a href="/admin/status" className="underline">Retry</a></p> : <>
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div className="border border-border-strong bg-surface-raised p-3">
              <p className="text-[10px] text-ink-tertiary">Total page views</p>
              <p className="mt-1 text-lg font-semibold text-ink">{usage.totalPageViews.toLocaleString()}</p>
            </div>
            <div className="border border-border-strong bg-surface-raised p-3">
              <p className="text-[10px] text-ink-tertiary">Unique logged-in users</p>
          ...
          {usage.topPaths.length > 0 && (
            <div className="mt-4">
              <p className="text-[10px] text-ink-tertiary">Top pages</p>
              <div className="mt-2 flex flex-col gap-1.5">
                {usage.topPaths.map((p) => (
                  <div key={p.path} className="flex items-center justify-between gap-3 text-xs">
                    ...
                    <span className="shrink-0 font-data text-ink-tertiary">{p.count.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {usage.eventCounts.length > 0 && (
            <div className="mt-4">
              <p className="text-[10px] text-ink-tertiary">Feature events</p>
              <div className="mt-2 flex flex-col gap-1.5">
                {usage.eventCounts.map((e) => (
                  <div key={e.eventType} className="flex items-center justify-between gap-3 text-xs">
                    ...
                    <span className="shrink-0 font-data text-ink-tertiary">{e.count.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {usage.totalPageViews === 0 && usage.eventCounts.length === 0 && (
        ...
        <div className="mt-6 border border-border bg-surface">
          <p className="border-b border-border px-4 py-3 font-headline text-sm uppercase text-ink-tertiary">
            Data sources
          </p>
                ...
                <div className="min-w-0">
                  ...
                  {report?.lastSuccessAt && (
                    <p className="mt-0.5 font-mono text-[9px] text-ink-tertiary">
                      Last success: {new Date(report.lastSuccessAt).toLocaleString("en-US", { timeZone: "America/Detroit", timeZoneName: "short" })}
                    </p>
                  )}
                </div>
          ...
          {events.length === 0 ? (
            ...
            events.map((e, i) => (
              <div key={i} className="border-b border-border px-4 py-3 text-sm text-ink-secondary last:border-0">
                <p className="font-data text-[9px] text-ink-tertiary">{new Date(e.at).toLocaleString("en-US", { timeZone: "America/Detroit", timeZoneName: "short" })}</p>
