...
import { ClipboardList } from "@/components/ui/SiteIcons";
...
interface DraftSettings {
  fullBoard?: SavedBoardPick[];
  teams: number; slot?: number;
    ...
    <main>
      ...
      <section className="py-10">
        <Container className="max-w-2xl">
          ...
          <div className="border border-border bg-surface">
            {draft.picks.map((pick) => (
              ...
              >
                ...
                <span className={`w-7 shrink-0 text-right font-data text-sm font-bold ${gradeTone(pick.grade)}`}>
                  {pick.grade}
                </span>
              </Link>
            ))}
          ...
          <SavedDraftBoard picks={draft.settings.fullBoard} slot={draft.settings.slot} teams={draft.settings.teams} format={draft.settings.qbFormat === "SUPERFLEX" ? (draft.settings.teFormat === "TEP" ? "SUPERFLEX_TEP" : "SUPERFLEX") : (draft.settings.teFormat === "TEP" ? "1QB_TEP" : "1QB")} />
      </Container>
      </section>
    </main>
  );
