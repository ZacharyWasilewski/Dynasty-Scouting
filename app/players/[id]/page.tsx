import { playerMetadata } from "@/lib/playerMetadata";
import { playerScoreStage } from "@/lib/scoreStage";
import { ComparePlayerLink } from "@/components/profile/ComparePlayerLink";
...
export async function generateMetadata({ params, searchParams }: {
  ...
  return playerMetadata(prospect, format);
}
...
function rankWithinPool(target: Prospect, pool: Prospect[], format: LeagueFormat): number | undefined {
  const ranked = pool.filter(p => Number.isFinite(scoreForFormat(p, format))).sort((a, b) => {
...
export default async function PlayerProfilePage({
  ...
  for (const fmt of RANK_FORMATS) {
    classRankByFormat[fmt] =
      classPool.length > 0 ? { rank: rankWithinPool(currentProspect, classPool, fmt), total: classPool.filter(p => Number.isFinite(scoreForFormat(p, fmt))).length } : undefined;
    positionRankByFormat[fmt] =
      positionPool.length > 0
        ? { rank: rankWithinPool(currentProspect, positionPool, fmt), total: positionPool.filter(p => Number.isFinite(scoreForFormat(p, fmt))).length }
        : undefined;
  }

  // Match the profile's score stage, position, tier and league format.
  const scoreStage = playerScoreStage(currentProspect);
  ...
  const hitRatesByFormatAndTier: Record<string, ReturnType<typeof computeHitRateByPositionAndTier>> = {
    "1QB": computeHitRateByPositionAndTier(allProspects, currentProspect.position as Position, { qbFormat: "1QB", tepFormat: "STANDARD" }, tierNames, scoreStage),
    "1QB_TEP": computeHitRateByPositionAndTier(allProspects, currentProspect.position as Position, { qbFormat: "1QB", tepFormat: "TEP" }, tierNames, scoreStage),
    SUPERFLEX: computeHitRateByPositionAndTier(allProspects, currentProspect.position as Position, { qbFormat: "SF", tepFormat: "STANDARD" }, tierNames, scoreStage),
    SUPERFLEX_TEP: computeHitRateByPositionAndTier(allProspects, currentProspect.position as Position, { qbFormat: "SF", tepFormat: "TEP" }, tierNames, scoreStage),
  };
  ...
  return (
    <main>
      <ProfileTabsProvider>
        ...
        >
          {/* Passed as children specifically so these three panels'
              content lives inside the same card as the Overview panel
              and the sticky tab bar above them all — see ProfileHeader's
              own comment on the children prop for why. */}

          {/* SCORES — now format-aware; see ModelScoresSection's own
              comment for why this had to become a client component. */}
          <ProfileTabPanel id="progression">
          ...
          {/* RESEARCH CONTEXT — keep related decision-support modules in the
              ...
              p-4/p-5/p-7/p-8, matching the card's other in-card blocks —
              see ModelScoresSection's identical comment on this same
              padding choice. */}
          <div className="border-t border-border bg-void p-4 py-8 sm:p-5 lg:p-7 lg:py-9 xl:p-8">
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(420px,0.95fr)] lg:items-start">
              {similarProspects.length > 0 ? (
                <ProfileTabPanel id="comps">
                  <div id="similar-prospects" className="min-w-0 scroll-mt-[calc(var(--nav-offset)+56px)]">
                    <div className="flex flex-wrap items-end justify-between gap-4">
                      <SectionHeading
                        eyebrow="Comparison Tool"
                        title="Similar prospects"
                        description="The closest model matches at this position, weighted by how much each score matters."
                      />
                      <ComparePlayerLink
                    ...
                    <div className="mt-5">
                      <PlayerComparison current={currentProspect} similar={similarProspects} allProspects={allProspects.filter(p => p.position === currentProspect.position)} weights={weights} />
                    </div>
                  </div>
                </ProfileTabPanel>
              ) : <div />}

              <ProfileTabPanel id="draft">
                <div id="draft-projection" className="min-w-0 scroll-mt-[calc(var(--nav-offset)+56px)]">
