...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error("[mock-drafts/:id] Failed to load saved draft:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to load this draft." },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error("[mock-drafts/:id] Failed to delete saved draft:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to delete this draft." },
      { status: 500 }
    );
  }
}
