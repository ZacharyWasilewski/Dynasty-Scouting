...
export async function POST(request: Request) {
  ...
  let body: { classYear?: unknown; prospectIds?: unknown };
  try {
  ...
  if (body.prospectIds !== undefined && (!Array.isArray(body.prospectIds) || body.prospectIds.length > 1000 || !body.prospectIds.every(id => typeof id === "string"))) return NextResponse.json({ error: "Invalid board." }, { status: 400 });
  const prospectIds = body.prospectIds as string[] | undefined ?? rows[0]?.prospect_ids;
