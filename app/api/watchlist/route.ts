...
export async function POST(request: Request) {
  ...
  let body: { prospectId?: unknown; saved?: unknown };
  try {
  ...
  // Explicit desired state makes retries safe, including a lost response.
  if (typeof body.saved === "boolean") {
    if (body.saved) await query(`INSERT INTO watchlist_items (user_id, prospect_id) VALUES ($1,$2) ON CONFLICT (user_id, prospect_id) DO NOTHING`, [user.id, prospectId]);
    else await query(`DELETE FROM watchlist_items WHERE user_id=$1 AND prospect_id=$2`, [user.id, prospectId]);
    return NextResponse.json({ prospectId, saved: body.saved });
  }
