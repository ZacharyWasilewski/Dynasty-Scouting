import { formatFromParam } from "@/lib/playerLinks";
...
export async function GET(request: Request, { params }: { params: { year: string } }) {
  ...
  const multipliers = Object.fromEntries(Object.entries(scales).map(([key, value]) => [key, value.multipliers]));
  const format = formatFromParam(new URL(request.url).searchParams.get("format"));
  const scores: Record<string, number> = {};
  for (const row of rows) {
    const prospect = prospects.find(p => p.id === row.prospect_id && String(p.draftClass) === params.year);
    const position = prospect && positionOf(prospect.position);
    if (!position) continue;
    const result = scoreWithOpportunityOverride(prospects, row.prospect_id, row.opportunity, multipliers, scales[position].opportunityWeight, format);
    if (result) scores[row.prospect_id] = result.ddScore;
  }
...
export async function PUT(request: Request, { params }: { params: { year: string } }) {
  ...
  const result = scoreWithOpportunityOverride(
    ...
    formatFromParam(new URL(request.url).searchParams.get("format"))
  );
  if (result === undefined) {
    return NextResponse.json(
